...
function showMessage_test($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	if (!class_exists('Language')) import('libraries.language');
	Language::read('core_lang_index');
	$lang	= Language::getLangContent();
	/**
 ...
 * @param int $time 停留时间
 */
function showDialog($message = '', $url = '', $alert_type = 'error', $extrajs = '', $time = 5){
...
function showDialog_test($message = '', $url = '', $alert_type = 'error', $extrajs = '', $time = 3){
	if (empty($_GET['inajax'])){
		if ($url == 'reload') $url = '';
		showMessage_test($message.$extrajs,$url,'html',$alert_type,1,$time*1000);
	}
	die;
	$message = str_replace("'", "\\'", strip_tags($message));
...
function Model($model = null, $base_path = null){
	...
	$class_name = $model_arr[count($model_arr)-1].'Model';
...
//取得用户显示名称
function getMemberShowName(){
	if($_SESSION["member_id"]){
		return (empty($_SESSION['org_id'])?"":$_SESSION['org_name'].'_').(empty($_SESSION['member_truename'])?$_SESSION['member_name']:$_SESSION['member_truename']);
	}
	return "";
}
/**
...
/**************图片验证码*********************/
//请求地址编码
function _getNchash($act, $op){
    return substr(md5(strtolower($act.$op)),0,8);
}
//设置验证码
function _set_code($hash,$code){
    if(empty($hash)||empty($code)){
        return false;
    }
    $data = array();
    $data['code'] = $code;
    $data['time'] = time();
    //刷新验证码
    $_SESSION[$hash.'_code'] = $data;
    return true;
}
//检测验证码,验证成功销毁验证码
function _check_code($hash,$code,$destory = false){
    if(empty($hash)){
        return false;
    }
    if(empty($code)){
    ...
    $ret = false;
    $data = $_SESSION[$hash.'_code'];
    if(!empty($data)&&is_array($data)&&$data['code'] == $code){
        $ret = true;
    }
    if($destory&&$ret){
        unset($_SESSION[$hash.'_code']);
    }
    return $ret;
}
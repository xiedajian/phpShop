...
class Sms {
    ...
    */
    public function send($mobile,$content,$filter = false) {
        $id = $this->_send_filter($mobile,$filter);
		$mobile_host_type=C('mobile_host_type');
		if($mobile_host_type==1) {
			$ret = $this->mysend_smsbao($mobile,$content);
		}
		if($mobile_host_type==2) {
			$ret =  $this->mysend_yunpian($mobile,$content);
		}
//        $ret = true;
        //发送成功状态修改
        if($ret&&$filter){
            $send_record_model = Model('send_code_record');
            $send_record_model->send_succ($id);
        }
		return $ret;
    }
	/*
	...
	*/
    ...
    //短信发送统一过滤
    private function _send_filter($mobile_no,$filter){
		//添加短信发送记录,并记录是否成功
		...
		if($filter){
			//添加记录
			$record_id = $send_record_model->add_record($mobile_no,$request_ip,$request_url);
            //是否可以发送
            $can_send = $send_record_model->can_send($mobile_no,$request_ip);
            if(!$can_send){
                exit(json_encode(array('code'=>1,'msg'=>'验证码发送太频繁，同一号码半小时内只能发送两条短信验证码！')));
            }
        }
        //可以发送添加发送记录
        $send_record_model->send_msg($request_url);
        return $record_id;
    }

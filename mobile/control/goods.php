...
class goodsControl extends mobileHomeControl{
    ...
    public function goods_listOp() {
        ...
        if(!empty($_GET['gc_id']) && intval($_GET['gc_id']) > 0) {
            ...
            $condition['goods_name|goods_jingle|goods_tag'] = array('like', '%' . $_GET['keyword'] . '%');
        }
        ...
        if(!$member_info['member_id']){
            ...
            $price['show_price_tip'] = 1;
        }else if($member_info['auth_statu']!=1){
            $price['show_price'] = false;
            $price['show_price_tip'] = 2;
        }
        output_data(array('goods_list' => $goods_list,'price'=>$price), mobile_page($page_count));
    }
    ...
    public function goods_detailOp() {
        ...
        //获取进货单总数
        if(!empty($_SESSION['member_id'])){
        	$goods_detail["cart_num"]=Model("cart")->getCartNum("db",array("buyer_id"=>$_SESSION['member_id']));
        	$model_member = Model('member');
        	$member_info = $model_member->getMemberInfoByID($_SESSION['member_id']);
            $org_info=$model_member->getMemberOrg(array('org_id'=>$member_info['org_id']),"org_name,auth_statu");
            $member_info['auth_statu']=$org_info['auth_statu'];
        ...
        // 商品详细信息
        $model_goods = Model('goods');
        $goods_detail = $model_goods->getGoodsDetail($goods_id,!empty($member_info)&&$member_info["member_level"]==2);
        if (empty($goods_detail)) {
            output_error('商品不存在');
        }
        //样品信息
        $sample_goods_info = $model_goods->getSampleGoodsInfo(array('goods_commonid'=>$goods_detail['goods_info']['goods_commonid']));
        $sample_goods_info['goods_mobile_rules'] = unserialize($sample_goods_info['goods_mobile_rules']);
        $goods_detail['sample_goods_info'] = $sample_goods_info;
        //推荐商品
        ...
        $goods_detail['show_price'] = true;
        if(!$member_info['member_id']){
            $goods_detail['show_price'] = false;
            $goods_detail['show_price_tip'] = 1;
        }else if($member_info['auth_statu']!=1){
            $goods_detail['show_price'] = false;
            $goods_detail['show_price_tip'] = 2;
        }
        //显示vip价
        $goods_detail['show_vip_price'] = false;
        $price = isset($goods_detail['goods_info']['promotion_price'])&&$goods_detail['goods_info']['promotion_price']>0 ? $goods_detail['goods_info']['promotion_price']:$goods_detail['goods_info']['goods_price'];
        if($goods_detail['goods_info']['goods_vip_price']>0&&$goods_detail['goods_info']['goods_vip_price']<$price&&$member_info['member_level']==2){
            $goods_detail['show_vip_price'] = true;
        }

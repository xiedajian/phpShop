  ...
  <div class="nc-appbar-tabs" id="appBarTabs">
    ...
    <div class="user-info" nctype="barUserInfo" style="display:none;"><i class="arrow"></i>
      ...
      <dl>
        ...
        <dd>当前等级：<strong nctype="barMemberGrade"><?php echo $output['member_info']['member_level_name'];?></strong></dd>
    ...
    <ul class="tools">
        <?php if ($_SESSION['is_login']) {?>
        <li><a href="javascript:void(0)" id="inform_show" class="chat">公告</a></li>
        <?php } ?>
        <li><a href="tencent://message/?uin=2785299647" target="_blank" id="chat_show_user" class="chat">在线客服<i id="new_msg" class="new_msg" style="display:none;"></i></a></li>
      <?php if (!$output['hidden_rtoolbar_cart']) { ?>
      <li><a href="javascript:void(0);" id="rtoolbar_cart" class="cart">进货单<i id="rtoobar_cart_count" class="new_msg" style="display:none;"></i></a></li>
      <?php } ?>
  ...
  <div class="nc-hidebar" id="ncHideBar">
    ...
    <?php if ($_SESSION['is_login']) {?>
        <div id="inform_content">
            <div id="inform_content_layer"></div>
            <div id="inform_content_main">
                <div id="inform_content_cancel"> x </div>
                <img width="750" height="379" src="<?php echo BASE_SITE_URL; ?>/shop/templates/default/images/notice_ipvp.png">
            </div>
        </div>
    <?php } ?>
...
<div class="public-top-layout w">
  <div class="topbar wrapper">
    <div class="user-entry">
      ...
      <a href="<?php echo urlShop('member','home');?>"><?php echo getMemberShowName();?></a>
      <?php if ($output['member_info']['level_name']){ ?>
      <div class="nc-grade-mini" style="cursor:pointer;"><?php echo $output['member_info']['member_level_name'];?></div>
      <?php } ?>
      ...
      <?php }else{?>
      <?php echo $lang['nc_hello'].$lang['nc_comma'].$lang['welcome_to_site'];?> <a href="<?php echo BASE_SITE_URL;?>" title="<?php echo $lang['homepage'];?>" alt="<?php echo $lang['homepage'];?>"><?php echo $output['setting_config']['site_name']; ?></a> <span>[<a href="<?php echo urlShop('login');?>"><?php echo $lang['nc_login'];?></a>]</span> <span>[<a href="<?php echo urlShop('login','ipvp_register');?>"><?php echo $lang['nc_register'];?></a>]</span>
      <?php }?>
    ...
    <div class="quick-menu">
      ...
      <dl>
        <dt><a href="<?php echo BASE_SITE_URL;?>/shop/index.php?act=article&op=show&article_id=63">关于我们</a></dt>
      </dl>
    </div>
  </div>
</div>
...
$(function() {
    ...
    $('#inform_show').click(function () {
        $('#inform_content').show();
        var $inform_content_main = $('#inform_content_main');
        var topPx = (screenHeight -  $inform_content_main.height()) / 2;
        var leftPx = (screenWidth - $inform_content_main.width()) / 2;
        $('#inform_content_layer, #inform_content').width(screenWidth).height(screenHeight);
        $('#inform_content_main').css({'top':topPx+'px', 'left':leftPx+'px'});
    });
    $('#inform_content_cancel').click(function () {
        $('#inform_content').hide();
    });
    //读取cookie
//    var res=document.cookie;
//    var is_show = res.indexOf('ipvp_show_login_inform') === -1;
//    if(is_show) {
//        $('#inform_show').trigger('click');
//        //添加cookie
//        var oDate = new Date();
//        oDate.setDate(oDate.getDate() + 1);
//        document.cookie = "ipvp_show_login_inform=1;expires=" + oDate+';path=/';
//    }
    <?php } ?>
});
</script>

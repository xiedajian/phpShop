...
<script type="text/javascript">
    var express_list =new Array();
    <?php if($output['express_list']){$i=0;foreach($output['express_list'] as $express){?>
    express_list[<?php echo $i++;?>]={id:'<?php echo $express['id'];?>',e_name:'<?php echo $express['e_name'];?>',e_code:'<?php echo $express['e_code'];?>'};
    <?php }}?>

    function selected_item(i){
        $('#express_search_box').val(express_list[i].e_name);
        $('#express_id').val(express_list[i].id);
        $('#select-box').hide();
    }
$(function(){
    $('#post_form1').validate({

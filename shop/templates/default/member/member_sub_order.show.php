                            ...
                            <dl>
                                ...
                                <dd>
                                    <div class="alert alert-info m0">
                                        ...
                                        <span title="查询物流" class="show_shipping" style="float: right;cursor: pointer" load="1" e_code="<?php echo $output['express_list'][$sub_order['express_id']]['e_code'];?>" shipping_code="<?php echo $sub_order['shipping_code'];?>">V</span>
                                    </div>

                                    <ul class="shipping_ul" style="display: none">
                                        <li>加载中...</li>
                                        <li style="display: none"><?php echo date("Y-m-d H:i:s",$sub_order['send_time']); ?>&nbsp;&nbsp;<?php echo $lang['member_show_seller_has_send'];?></li>
                                    </ul>
...
<script>
    $('.show_shipping').on('click',function(){
       ...
       var e_code = $(this).attr('e_code');
        var shipping_code = $(this).attr('shipping_code');
        var load = $(this).attr('load');
        var ul = $(this).closest('dd').find('.shipping_ul');
        if(load==1){
            $(this).attr('load',0);
            $.getJSON('index.php?act=member_order&op=get_express&e_code='+e_code+'&shipping_code='+shipping_code,function(data){
                var first = ul.find('li:hidden').html();
                if(data){
                    data = first+'</li><li>'+data.join('</li><li>');
                    ul.html('<li>'+data+'</li>');
                }else{
                    ul.html(first);
                }

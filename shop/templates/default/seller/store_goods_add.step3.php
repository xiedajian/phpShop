...
<form method="post" id="goods_image" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_image'); } else { echo urlShop('store_goods_add', 'save_image');}?>">
  ...
  <div class="ncsc-form-goods-pic">
    <div class="container">
      ...
      <div class="ncsc-goodspic-list color-container">
        <div class="title">
    ...
    <?php if(!empty($output["spec_goods"])){?>
      <style type="text/css">
        .spec-goods-pic-box .upload-thumb{position:static!important;width:140px!important;}
        .spec-goods-pic-box .upload-thumb img{max-width:130px!important;max-height:130px!important;margin: 5px;}
        .spec-goods-pic-box  .ncsc-goods-spec-info{text-align:center;line-height:20px;padding:3px 5px;}
        .spec-goods-pic-box  ul li{height:inherit!important;border-right:1px solid #E6E6E6;border-left:none!important;}
        .spec-goods-pic-box  .ncsc-upload-btn{position:static!important;}
        .spec-goods-pic-box .goods-image-thumb{border:none!important;}
        .spec-goods-pic-box .goods-image-thumb-selected img{border:1px solid #28B779;width:128px;}
      </style>
     <div id="spec_goods_pic_box" class="container">
      
      <div class="ncsc-goodspic-list spec-goods-pic-box spec-container">
        <div class="title">
        <h3>商品规格图片</h3></div>
        <ul>
          <?php foreach ($output['spec_goods'] as $i=>$value) {?>
          <li class="ncsc-goodspic-upload">
            <div class="upload-thumb goods-image-thumb <?php echo $i==0?"goods-image-thumb-selected":"";?>"><img src="<?php echo cthumb($value["goods_image"], 180);?>" nctype="file_s<?php echo $value['goods_id'];?>">
              <input type="hidden" name="goods_img[<?php echo $value['goods_id'];?>]" value="<?php echo $value['goods_image'];?>" nctype="file_s<?php echo $value['goods_id'];?>">
            </div>
            <div class="ncsc-goods-spec-info">
               <?php if($value["is_custom_spec"]){?>
                 <?php echo $output["common_spec"]["custom_spec_name"];?>: <?php echo $value["custom_goods_spec"];?>
               <?php }else{?>
                  <?php $spec_array=array_values((array)unserialize($value['goods_spec']));?>
                  <?php echo implode(' ',$spec_array);?>
               <?php }?>
            </div>
            <div class="ncsc-upload-btn">
            <a href="javascript:void(0);"><span><input type="file" hidefocus="true" size="1" class="input-file" name="file_s<?php echo $value['goods_id'];?>" id="file_s<?php echo $value['goods_id'];?>"></span><p><i class="icon-upload-alt"></i>上传</p>
              </a>
             </div>
          </li>
          <?php }?>
        ...
        <div class="ncsc-select-album">
          <a class="ncsc-btn" href="index.php?act=store_album&op=pic_list&item=goods_image" nctype="select-goods-spec-pic"><i class="icon-picture"></i>从图片空间选择</a>
          <a href="javascript:void(0);" nctype="close_album" class="ncsc-btn ml5" style="display: none;"><i class=" icon-circle-arrow-up"></i>关闭相册</a>
        </div>
        <div nctype="album-goods-spec-pic"></div>
      </div>
      
    </div>
    <?php } ?>
    <div style="  width: 235px;position: absolute;right: 20px;"><div class="alert alert-info alert-block">
...
$(function(){
    ...
    <?php }?>
    <?php if(!empty($output["spec_goods"])){?>
    $('a[nctype="select-goods-spec-pic"]').ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:SHOP_TEMPLATES_URL+"/images/loading.gif",
        target:'div[nctype="album-goods-spec-pic"]'
    }).click(function(){
        $(this).hide();
        $(this).next().show();
    });
    $("#spec_goods_pic_box .goods-image-thumb").click(function(){
    	$("#spec_goods_pic_box .goods-image-thumb").removeClass("goods-image-thumb-selected");
    	$(this).addClass("goods-image-thumb-selected");
     });
<?php }?>
});
</script> 

<div class="goods-gallery add-step2"><a class='sample_demo' id="select_submit" href="index.php?act=store_album&op=pic_list&item=goods" style="display:none;"><?php echo $lang['nc_submit'];?></a>
  ...
  <ul class="list">
    <?php foreach ($output['pic_list'] as $v){?>
      <li onclick="insert_img('<?php echo $v['apic_cover'];?>','<?php echo thumb($v, 240);?>');" style="font-size: 12px;height: 120px;"><a href="JavaScript:void(0);"><img src="<?php echo thumb($v, 240);?>" title='<?php echo $v['apic_name']?>'/></a><div style="margin-top: 2px;text-align:center;line-height: 14px;word-wrap: break-word;"><?php echo $v['apic_name']?></div></li>
    <?php }?>
  </ul>
  <?php }else{?>

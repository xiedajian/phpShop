  ...
  <form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
    ...
    <div class="ncsc-form-goods">
      ...
      <dl>
        <dt>商品标签：</dt>
        <dd>
         <input name="g_tag" type="text" class="text w400" value="<?php echo $output['goods']['goods_tag']; ?>" />
        </dd>
      </dl>
      <dl>
      ...
      <dl>
      <dl>
          <dt nc_type="no_spec"><?php echo 'VIP价格'.$lang['nc_colon'];?></dt>
          <dd nc_type="no_spec">
              <input name="g_vip_price" value="<?php echo $output['goods']['goods_vip_price']; ?>" type="text"  class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
              <p class="hint"><?php echo $lang['store_goods_index_store_price_help'];?>，且不能高于市场价。<br>
                  此价格为VIP会员价格，如果商品存在规格，该价格显示最低价格。</p>
          </dd>
      </dl>
      <dl>
      ...
      <dl>
        <dt><i class="required">*</i>拼单成功数<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="p_success_num" value="<?php echo $output['goods']['pindan_success_num']; ?>" type="text" class="text w60" /><span></span>
          <p class="hint">设为0不参与拼单。</p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>拼单成功返利<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="p_common_rebates" value="<?php echo $output['goods']['pindan_common_rebates']; ?>" type="text" class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em> <span></span>
          <p class="hint">当没有设置规格时商品的返利值,有规格时会根据规格设置的返利值进行拼单成功返利。</p>
        </dd>
      </dl>
      <dl>
        <dt><i class="required">*</i>拼单周期<?php echo $lang['nc_colon'];?></dt>
        <dd>
          <input name="p_cycle" value="<?php echo $output['goods']['pindan_cycle']; ?>" type="text" class="text w60" /><em class="add-on">小时</em> <span></span>
          <p class="hint">周期内的拼单。</p>
        </dd>
      </dl>
        ...
        <dd class="spec-dd">
          <table border="0" cellpadding="0" cellspacing="0" class="spec_table">
            ...
            <th nctype="spec_name_<?php echo $k;?>"><?php if (isset($output['goods']['spec_name'][$k])) { echo $output['goods']['spec_name'][$k];} else {echo $val['sp_name'];}?></th>
              ...
              <th class="w90"><span class="red">*</span>售价
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  <div class="batch-input" style="display:none;">
                    <h6>批量设置价格：</h6>
                    <a href="javascript:void(0)" class="close">X</a>
              ...
              <th class="w90"><span class="red"></span>VIP价格
                  <div class="batch"><i class="icon-edit" title="批量操作"></i>
                      <div class="batch-input" style="display:none;">
                          <h6>批量设置价格：</h6>
                          <a href="javascript:void(0)" class="close">X</a>
              ...
              <th class="w90"><span class="red"></span>拼单返利
                  <div class="batch"><i class="icon-edit" title="批量操作"></i>
                      <div class="batch-input" style="display:none;">
                          <h6>批量设置返利：</h6>
                          <a href="javascript:void(0)" class="close">X</a>
                          <input name="" type="text" class="text price" />
                          <a href="javascript:void(0)" class="ncsc-btn-mini" data-type="rebates">设置</a><span class="arrow"></span></div>
                  </div></th>
              ...
              <th class="w70">预警值
                <div class="batch"><i class="icon-edit" title="批量操作"></i>
                  ...
                  <th class="w60">起订量</th>
                  <th class="w50"><?php echo $lang['store_goods_index_goods_no'];?></th>
                </thead>
      ...
      <dl>
       <dt><i class="required">*</i>最小起订量<?php echo $lang['nc_colon'];?></dt>
       <dd>
        <input name="g_moq" value="<?php echo $output['goods']['goods_common_moq'];?>" type="text" class="text w100" />
        <span></span>
       </dd>
      </dl>
      <dl>
...
$(function(){
    ...
    $('#goods_form').validate({
        ...
        messages : {
            ...
            g_moq    : {
                required    : '<i class="icon-exclamation-sign"></i>最小起订量为空',
                digits      : '<i class="icon-exclamation-sign"></i>请正确输入正整数',
                min         : '<i class="icon-exclamation-sign"></i>最小起订量为1',
                max         : '<i class="icon-exclamation-sign"></i>数值大小在1~9999999'
            },
            p_success_num    :  {
                required    : '<i class="icon-exclamation-sign"></i>拼单成功数为空',
             ...
             p_common_rebates    :  {
                 required    : '<i class="icon-exclamation-sign"></i>返利金额为空',
                 min         : '<i class="icon-exclamation-sign"></i>最小值为0',
                 max         : '<i class="icon-exclamation-sign"></i>数值太大',
              },
              p_cycle    :  {
                  required    : '<i class="icon-exclamation-sign"></i>拼单周期为空',
                  digits      : '<i class="icon-exclamation-sign"></i>请正确输入',
...
function goods_stock_set(){
    ...
    if(str == '<tr>'){
        ...
        $('input[name="g_vip_price"]').removeAttr('readonly').css('background','#E7E7E7 none');
        ...
        $('tbody[nc_type="spec_table"]').empty().html(str)
            .find('input[nc_type]').each(function(){
                ...
                if ($(this).attr('data_type') == 'rebates' && $(this).val() == ''){
                    $(this).val($('input[name="p_common_rebates"]').val());
                }
                ...
                if ($(this).attr('data_type') == 'moq' && $(this).val() == ''){
                    $(this).val('1');
                }
            }).end()
            ...
            .find('input[data_type="marketprice"]').change(function(){
              var price = $(this).closest('tr').find('input[data_type="price"]').val();
              if($(this).val()-price<0){
                  $(this).focus();
                show_spc_tip('市场价不能小于售价！');
              }
            }).end()
            .find('input[data_type="price"]').change(function(){
                var marketprice = $(this).closest('tr').find('input[data_type="marketprice"]').val();
                if($(this).val()-marketprice>0){
                    $(this).focus();
                  show_spc_tip('售价不能大于市场价！');
                }
                computePrice();     // 价格计算
            }).end()
...
function recursionSpec($len,$sign) {
    if($len < $sign){
        ...
        echo "str +='<td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][marketprice]\" data_type=\"marketprice\" nc_type=\"'+spec_bunch+'|marketprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][price]\" data_type=\"price\" nc_type=\"'+spec_bunch+'|price\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][vipprice]\" data_type=\"vipprice\" nc_type=\"'+spec_bunch+'|vipprice\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text price\" type=\"text\" name=\"spec['+spec_bunch+'][rebates]\" data_type=\"rebates\" nc_type=\"'+spec_bunch+'|rebates\" value=\"\" /><em class=\"add-on\"><i class=\"icon-renminbi\"></i></em></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][stock]\" data_type=\"stock\" nc_type=\"'+spec_bunch+'|stock\" value=\"\" /></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][alarm]\" data_type=\"alarm\" nc_type=\"'+spec_bunch+'|alarm\" value=\"\" /></td><td><input class=\"text stock\" type=\"text\" name=\"spec['+spec_bunch+'][moq]\" data_type=\"moq\" nc_type=\"'+spec_bunch+'|moq\" value=\"\" /></td><td><input class=\"text sku\" type=\"text\" name=\"spec['+spec_bunch+'][sku]\" nc_type=\"'+spec_bunch+'|sku\" value=\"\" /></td></tr>';\n";
        for($i=0; $i< $len; $i++){
            echo "}\n";
        }
    }
...
<?php }?>
  ...
  function show_spc_tip(msg){
      if($('.spec-dd').find('div.tips').length==0){
        $('.spec-dd').prepend('<div class="tips"></div>')
      }
      $('.spec-dd').find('div.tips').html(msg).show(500).delay(3000).hide(100);
  }
</script> 

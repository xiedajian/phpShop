...
<div class="ncc-main">
  ...
  <form action="<?php echo urlShop('buy','buy_step1');?>" method="POST" id="form_buy" name="form_buy">
    ...
    <table class="ncc-table-style" nc_type="table_cart" style="margin-bottom: 0px;border-bottom: 0px;">
      <thead>
        <tr>
          <th class="w50"><label>
      ...
      <tbody>
        <tr>
            <th style="text-align: center"><input type="checkbox" class="store_select_all" checked="true" nc_type="eachGoodsCheckBox"></th>
        ...
        <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" nc_group="<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo $cart_info['state'] ? '' : 'item_disabled';?>">
          ...
          <td class="tl" <?php if ($cart_info['bl_id'] != '0') {?>colspan="2"<?php }?>><dl class="ncc-goods-info">
              <dt><a href="<?php echo urlShop('goods','index',array('goods_id'=>$cart_info['goods_id']));?>" target="_blank"><?php echo $cart_info['goods_name']; ?></a>
                  <?php  if($cart_info['goods_tag']=='保税仓商品'){ ?>
                      <em class="bondedwarehouse">保税仓发货</em>
                  <?php  }?></dt>
    ...
    <style>
        .cart-bottom{background-color: #E5E5E5;font-size: 14px;height: 60px; line-height: 60px;}
        .cart-bottom .left{float: left;margin-left: 20px;cursor: pointer}
        .cart-bottom .right{float: right;margin-right: 20px;}
        .cart-bottom-fixed{position: fixed;bottom: 0px;width: 1000px;}
        .next-btn{
            padding: 8px 12px;
   ...
   <div class="cart-bottom">
       <span class="left"><input type="checkbox" checked value="1" id="selectAll"></span>
       <span class="left"><label for="selectAll">全选</label></span>
       <span class="left" id="mul_del">删除</span>

       <div class="right next-btn"id="next_submit" href="javascript:void(0)"><i class="icon-pencil"></i><?php echo $lang['cart_index_input_next'].$lang['cart_index_ensure_info'];?></div>
       <span class="right"><?php echo $lang['cart_index_goods_sumary'];?><em id="cartTotal"><?php echo $output['cart_totals']; ?></em><?php echo $lang['currency_zh'];?></span>
   </div>
  <!-- 猜你喜欢 -->
...
$(function(){
    ...
    $(window).scroll(function(){
        var top =$('#bottom_line').offset().top;
        if($(window).scrollTop()+$(window).height()-$('.cart-bottom').height()<top){

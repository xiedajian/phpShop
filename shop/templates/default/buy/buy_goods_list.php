  ...
  <table class="ncc-table-style">
    ...
    <tbody>
      <tr>
        <th colspan="20"><strong>店铺：<a href="<?php echo urlShop('show_store','index',array('store_id'=>$store_id));?>"><?php echo $cart_list[0]['store_name']; ?></a></strong> <span member_id="<?php echo $output['store_list'][$store_id]['member_id'];?>"></span>
          <div class="store-sale">
            ...
            <a href="<?php echo urlShop('goods','index',array('goods_id'=>$goods_info['goods_id']));?>" target="_blank" class="ncc-store-gift" title="<?php echo $goods_info['goods_name']; ?>">
                <img src="<?php echo cthumb($goods_info['goods_image'],60,$store_id);?>" alt="<?php echo $goods_info['goods_name']; ?>" /></a>
            <?php } ?>
            <?php  } ?>
            <?php } ?>
          </div></th>
      ...
      <tr id="cart_item_<?php echo $cart_info['cart_id'];?>" class="shop-list <?php echo ($cart_info['state'] && $cart_info['storage_state']) ? '' : 'item_disabled';?>">
        ...
        <td class="w60"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$cart_info['goods_id']));?>" target="_blank" class="ncc-goods-thumb">
                <img src="<?php echo thumb($cart_info,60);?>" alt="<?php echo $cart_info['goods_name']; ?>" /></a>
           </td>
        <?php } ?>
      ...
      <tr class="shop-list <?php echo $cart_info['state'] && $cart_info['storage_state'] ? '' : 'item_disabled';?>">
        <td></td>
        <td class="w60"><a href="<?php echo urlShop('goods','index',array('goods_id'=>$goods_info['goods_id']));?>" target="_blank" class="ncc-goods-thumb">
                <img src="<?php echo cthumb($goods_info['goods_image'],60,$store_id);?>" alt="<?php echo $goods_info['goods_name']; ?>" /></a>
        ...
        <td class="tl"><dl class="ncc-goods-info">
          ...
          </dl><?php  if($goods_info['goods_tag']=='保税仓商品'){ ?>
                <em style="color: red">(保税仓发货)</em>
            <?php  }?></td>
      ...
      <tr>
        ...
        <td class="tl" colspan="2">
            <div style="position: relative"> 买家留言：
            ...
            <?php  foreach ($cart_list as $v ) {
                if($v['goods_tag']=='保税仓商品'){
                    $cart_list['is_bondedwarehouse'] =true;
                    break;
                }
            }if($cart_list['is_bondedwarehouse']==1){?>
                <div class="help-tip">
                    <p>尊敬的拼单网客户，您好。根据海关相关政策规定，每2件保税仓商品对应一套身份信息，请您在留言中正确备注身份信息，为避免耽误您接收商品，身份证号码请填写正确，证件号码必须与证件姓名完全对应。如有疑问请联系在线客服或者拨打我们的客服热线4000908982。
                    </p>
                </div>
            <?php }?>
            </div>

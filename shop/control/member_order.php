...
class member_orderControl extends BaseMemberControl {
    ...
    public function indexOp() {
        ...
        foreach ($order_list as $order_id => $order) {
            ...
            foreach ($order['extend_order_goods'] as $value) {
                ...
                $value['goods_type_cn'] =  orderPayType($value['goods_pay_type'],$value['goods_type']);
                $value['goods_pay_type'] = $value['goods_pay_type'];
	...
	private function profile_menu($menu_key='') {
	    ...
	    $menu_array = array(
            ...
            array('menu_key'=>'charge_order','menu_name'=>'付费许可订单','menu_url'=>'index.php?act=member_order&op=charge_order'),
    ...
    public function show_sub_orderOp(){
        $model_order = Model('order');
        $condition['buyer_id'] = $_SESSION['member_id'];
        ...
        foreach($order_info['extend_order_goods'] as $value){
            ...
            $value['goods_url'] = urlShop('goods','index',array('goods_id'=>$value['goods_id']));
            if ($value['goods_type'] == 5) {
                $order_info['zengpin_list'][] = $value;//赠品
            } else {
        ...
        $list = $model_order->getSubOrderList(array('order_id'=>intval($_GET['order_id'])));
        foreach($list as $order){
            ...
            $sub_order_list[] = $order;
        }
        $express = rkcache('express',true);
        Tpl::output('sub_order_list',$sub_order_list);
        Tpl::output('order_info',$order_info);
        Tpl::output('express_list',$express);
        Tpl::showpage('member_sub_order.show');
    }
    /*
     * 付费许可订单页面
     * */
    public function charge_orderOp(){
        $model_order = Model('order');
        //搜索
        $condition = array();
        $condition['buyer_id'] = $_SESSION['member_id'];
        ...
        $order_list = $model_order->getOrderList($condition, 20, '*', 'add_time desc','');
       for($i=0;$i<sizeof($order_list);$i++){
           $order_list[$i]["charge_data"]=unserialize($order_list[$i]["charge_remark"]);
           $order_list[$i]["order_no_v2"]=substr_replace($order_list[$i]["order_sn"],"xxxxxxxx",4,8);
           if($order_list[$i]["order_state"]==10){
               $endtime=date('Y-m-d',strtotime('+3 day',$order_list[$i]["add_time"]));
               $times=strtotime($endtime)+60*60*2;//因为我们系统的定时任务我半夜2点执行的，这边再给他加上两个小时。
               $times=$times-time();
               $d=floor($times/(24*60*60));
               $times=$times-24*60*60*$d;
               $h=floor($times/(60*60));
               $times=$times-60*60*$h;
               $m=floor($times/60);
               $days_data="";
             ...
             $order_list[$i]["days_data"]=$days_data;
           }
       }
        self::profile_menu('charge_order');
        Tpl::output('show_page',$model_order->showpage());
        Tpl::output('order_list',$order_list);
        Tpl::showpage('charge_order');
    }
    public  function charge_order_infoOp(){
        $model_order=Model('order');
        $order_sn=$_GET["order_sn"];
        $order_info=$model_order->getorder_info(array("order_sn"=>$order_sn));
        $order_info["order_no_v2"]=substr_replace($order_info["order_sn"],"xxxxxxxx",4,8);
        $order_info["charge_data"]=unserialize($order_info["charge_remark"]);
        $price=$order_info["order_amount"]/($order_info["charge_data"]["licences"]* $order_info["charge_data"]["licences_months"]);
        $order_info["price"]=$price;
        if($order_info["order_state"]==10){
            $endtime=date('Y-m-d',strtotime('+3 day',$order_info["add_time"]));
            $times=strtotime($endtime)+60*60*2;//因为我们系统的定时任务我半夜2点执行的，这边再给他加上两个小时。
            $times=$times-time();
            $d=floor($times/(24*60*60));
            $times=$times-24*60*60*$d;
            ...
            $order_info["days_data"]=$days_data;
        }
        Tpl::output('order_info',$order_info);
        Tpl::showpage('charge_order_info',null_layout);
        exit();
    }
//删除缴费订单
    public function charge_delete_orderOp(){
        $model_order=Model('order');
        $order_id=$_GET["order_id"];
        $data=array();
        $data["delete_state"]=2;
        $result=$model_order->editOrder($data,array("order_id"=>$order_id));
        var_dump($result);
        if(!$result) {
            showDialog("删除失败",'','error');
        } else {
            showDialog("删除成功",'reload','js');
        }
    }

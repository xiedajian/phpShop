...
class Control{
     ...
     * 检测是否显示价格
     */
    protected  function show_price(){
        //显示价格
        $show_price = true;
        if($_SESSION['is_login']!=1||empty($_SESSION['member_id'])){
            $show_price = false;
            $show_price_short_tip = '登录后显示价格';
            $show_price_tip = '请先登录后查看价格&nbsp;<a href="index.php?act=login" style="color: #E1017E;">马上登录</a>';
        }
        else if($_SESSION['auth_statu']!=1){
            $show_price = false;
            $show_price_short_tip = '认证后显示价格';
            $show_price_tip = '请先认证后查看价格&nbsp;<a href="index.php?act=member_org&op=index" style="color: #E1017E;">马上认证</a>';
        }
        return array('show_price'=>$show_price,'show_price_tip'=>$show_price_tip,'show_price_short_tip'=>$show_price_short_tip);
    }
    /**
     * 检查短消息数量
     *
    ...
    protected function getMemberAndGradeInfo($is_return = false){
        ...
        if($_SESSION['member_id']) {
            ...
            $member_info['member_level_name'] = 'V'.($member_info['member_level']-1);
            if ($member_info){
    ...
    protected function checkLogin(){
        if ($_SESSION['is_login'] !== '1'){
            if($_SESSION['member_group']==2){//分销商
                header('location:'.UMALL_SITE_URL);exit;
            }
        ...
        $menu_list = array(
			...
			//'member_flea'     => array('name' => '我的闲置', 'url'=>urlShop('member_flea', 'index')),
                ...
                'info' => array('name' => '资料管理', 'child' => array(
                		'zhiyoubao'=> array('name' => '直邮宝服务', 'url'=>urlShop('zhiyoubao', 'index')),
                        'member_information'=> array('name' => '账户信息', 'url'=>urlShop('member_information', 'member')),
                		'member_information'=> array('name' => '店铺认证', 'url'=>urlShop('member_org', 'index')),

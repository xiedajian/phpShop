...
class predepositControl extends BaseMemberControl {
	...
	public function pindan_rebates_logOp(){
		self::profile_menu('','pindan_rebates_log');
		$pindan_model=Model("pindan");
		$list=$pindan_model->getPindanRebatesList(array("member_id"=>$_SESSION['member_id'],"rebates_state"=>1),"*","rebates_time desc",20);
		Tpl::output('show_page',$pindan_model->showpage());
		Tpl::output('list', $list);
		Tpl::showpage('member_rebates_log.list');
	}
	/**
	 * 用户中心右边，小导航
	 *
	...
	private function profile_menu($menu_type,$menu_key=''){
		$menu_array	= array(
			...
			array(
				'menu_key' => 'pindan_rebates_log',
				'menu_name' => '返现明细',
				'menu_url' => 'index.php?act=predeposit&op=pindan_rebates_log',
			)
		);
		switch ($menu_type) {
			case 'rechargeinfo':

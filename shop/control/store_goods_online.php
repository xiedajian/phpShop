...
class store_goods_onlineControl extends BaseSellerControl {
    ...
    public function edit_goodsOp() {
        ...
        // 取得商品规格的输入值
        $goods_array = $model_goods->getGoodsList($where, 'goods_id,goods_marketprice,goods_price,goods_vip_price,goods_storage,goods_serial,goods_storage_alarm,goods_moq,goods_spec,pindan_goods_rebates');
        $sp_value = array();
        if (is_array($goods_array) && !empty($goods_array)) {
            ...
            foreach ( $goods_array as $k => $v ) {
                ...
                if (!empty($a)) {
                    ...
                    $sp_value ['i_' . $id . '|vipprice'] = $v['goods_vip_price'];
                    ...
                    $sp_value ['i_' . $id . '|moq'] = $v['goods_moq'];
                    $sp_value ['i_' . $id . '|rebates'] = $v['pindan_goods_rebates'];
                }
            }
    ...
    public function edit_save_goodsOp() {
        ...
        $update_common['goods_tag']          = $_POST['g_tag'];
        ...
        $update_common['goods_vip_price']        = floatval($_POST['g_vip_price']);
        ...
        $update_common['goods_common_moq']   = intval($_POST['g_moq']);
        $update_common['goods_attr']         = serialize($_POST['attr']);
        $update_common['goods_body']         = $_POST['g_body'];
        $update_common['pindan_common_rebates']         = floatval($_POST['p_common_rebates']);
        $update_common['pindan_success_num']         =intval($_POST['p_success_num']);
        $update_common['pindan_cycle']         = intval($_POST['p_cycle']);
        // 序列化保存手机端商品描述数据
        if ($_POST['m_body'] != '') {
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                if ($value['marketprice'] < $value['price']) {
                    showDialog('售价不能小于市场价！','','error',"$($('input[data_type=\"marketprice\"]')[0]).focus();show_spc_tip('售价不能大于市场价！');",2);
                    exit;
                }
            }
        }
        ...
        if (is_array ( $_POST ['spec'] )) {
            foreach ($_POST['spec'] as $value) {
                ...
                if (!empty($goods_info)) {
                    ...
                    $update['goods_tag']         = $update_common['goods_tag'];
                    ...
                    $update['goods_vip_price']       = $value['vipprice'];
                    $update['pindan_goods_rebates']       =floatval($value['rebates']);
                    ...
                    $update['goods_moq']         = intval($value['moq']);
		    ...
		    // 生成商品二维码
                    ...
                    $insert['goods_tag']         = $update_common['goods_tag'];
                    ...
                    $insert['goods_vip_price']       = $value['vipprice'];
                    $insert['pindan_goods_rebates']       = floatval($value['rebates']);
                    ...
                    $insert['goods_moq']         = intval($value['moq']);  
            ...
            if (!empty($goods_info)) {
                ...
                $update['goods_vip_price']       = $update_common['goods_vip_price'];
                $update['pindan_goods_rebates']  = floatval($update_common['pindan_common_rebates']);
                ...
                $update['goods_moq']         = $update_common['goods_common_moq']; 
		 ...
		 // 生成商品二维码
                ...
                $insert['goods_vip_price']       = $update_common['goods_vip_price'];
                $insert['pindan_goods_rebates']  = floatval($update_common['pindan_common_rebates']);
                ...
                $insert['goods_moq']         = $update_common['goods_common_moq']; 
        ...
        $pindan_id=0;
        $pindan_info=array();
        $model_pindan=Model("pindan");
        $common_pindan=$model_goods->getGoodeCommonInfo(array("goods_commonid"=>$common_id),"pindan_id");
        if(!empty($common_pindan)&&!empty($common_pindan["pindan_id"])){
        	$pindan_id=$common_pindan["pindan_id"];
        	$pindan_info["success_num"]=$update_common['pindan_success_num'];
        	$pindan_info["end_time"]=array("exp","start_time+".$update_common['pindan_cycle']*60*60);
        }else if($common_pindan["pindan_id"]==0){//没有拼单活动创建拼单
        	$pindan_info["goods_commonid"]=$common_id;
        	$pindan_info["start_time"]=TIMESTAMP;
        	$pindan_info["end_time"]=TIMESTAMP+$update_common['pindan_cycle']*60*60;
        	$pindan_info["success_num"]=$update_common['pindan_success_num'];
        }
        // 商品加入上架队列
        if (isset($_POST['starttime'])) {
            ...
            if ($selltime > TIMESTAMP) {
                ...
                $pindan_info["start_time"]=$selltime;
                $pindan_info["end_time"]=$selltime+$update_common['pindan_cycle']*60*60;
            }
        }
        if($pindan_id>0){
        	if($update_common['pindan_success_num']>0){
        	    $model_pindan->editPindan(array("pindan_id"=>$pindan_id),$pindan_info);
        	}else{
        		$update_common["pindan_id"]=0;//去除拼单
        	}
        }else{
        	if($update_common['pindan_success_num']>0){//大于0创建拼单 0代表不拼单
        	    $model_pindan->createPindan($pindan_info);
        	}
        }
        // 添加操作日志
    ...
    public function edit_imageOp() {
        ...
        $specgoods=$model_goods->getGoodsSpecListByCommonId($common_id);
        if(!empty($specgoods)&&count($specgoods)>1){
        	Tpl::output('spec_goods', $specgoods);
        }
    ...
    public function edit_save_imageOp() {
        if (chksubmit()) {
            ...
            //规格产品图片修改
            if(!empty($_POST['goods_img'])&&is_array($_POST['goods_img'])){
            	foreach ($_POST['goods_img'] as $goods_id=>$img){
            		$model_goods->editGoodsById(array("goods_image"=>$img),$goods_id);
            	}
            }
            if ($rs) {
            // 添加操作日志
    ...
    private function profile_menu($menu_type,$menu_key, $allow_promotion = array()) {
        ...
        switch ($menu_type) {
            ...
            case 'edit_detail':
                if ($allow_promotion['lock'] === false) {
                    $menu_array = array(
                        ...
                        array('menu_key' => 'edit_samples',   'menu_name' => '编辑样品', 'menu_url' => urlShop('store_goods_online', 'edit_samples', array('commonid' => $_GET['commonid'], 'ref_url' => ($_GET['ref_url'] ? $_GET['ref_url'] : getReferer())))),
                    );
                }
            ...
            case 'onlybuy_power':
            	$menu_array = array(
            		array('menu_key' => 'goods_list',    'menu_name' => '出售中的商品', 'menu_url' => urlShop('store_goods_online', 'index')),
            		array('menu_key' => 'onlybuy_power',    'menu_name' => '独家经销权', 'menu_url' =>'#')
            	);
            	break;
        }
    ...
    /*样品编辑*/
    public function edit_samplesOp(){
        $common_id = intval($_GET['commonid']);
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), '', 'html', 'error');
        }
        ...
        $sample_goods_info = $model_goods->getSampleGoodsInfo(array('goods_commonid'=>$common_id));
        if ($sample_goods_info['goods_mobile_rules'] != '') {
            $sample_goods_info['mb_body'] = unserialize($sample_goods_info['goods_mobile_rules']);
            $sample_goods_info['goods_mobile_rules'] = json_encode($sample_goods_info['mb_body']);
        }
        $menu_promotion = array(
            'lock' => $goodscommon_info['goods_lock'] == 1 ? true : false,
            'gift' => $model_goods->checkGoodsIfAllowGift($goodscommon_info),
        ...
        $this->profile_menu('edit_detail','edit_samples', $menu_promotion);
        Tpl::output('sample_goods_info',$sample_goods_info);
        Tpl::output('goods',$goodscommon_info);
        Tpl::output('edit_goods_sign', true);
        Tpl::showpage('store_sample_goods_edit');
    }
    /*样品保存*/
    public function save_samplesOp(){
        $common_id = intval($_POST['commonid']);
        if ($common_id <= 0) {
            showMessage(L('wrong_argument'), '', 'html', 'error');
        }
        ...
        do{
            ...
            //样品价
            $sample_goods_price = doubleval($_POST['sample_goods_price']);
            if(!$sample_goods_price){
                $error = '样品价不能为空';break;
            }
            //数量限制
            $num_limit = intval($_POST['num_limit']);
            if(!$num_limit||$num_limit>$goodscommon_info['goods_common_moq']){
                $error = '数量限制不能超过最小起订量';break;
            }
            //运费
            if($_POST['freight']==0){
                $transport_id= 0;
                $freight = doubleval($_POST['g_freight']);
            }else{
                $freight = 0;
                $transport_id = intval($_POST['transport_id']);
                $transport_title = $_POST['transport_title'];
                if(!$transport_id||!$transport_title){
                    $error = '运费模板不能为空';break;
                }
            }
        }while(false);
        if ($error != '') {
        ...
        if($open==1){
            ...
            $update['goods_commonid'] = $common_id;
            $update['enable'] = 1;
            $update['goods_name'] = $goodscommon_info['goods_name'];
            $update['goods_price'] = $sample_goods_price;
            $update['num_limit'] = $num_limit;
            $update['transport_id'] = $transport_id;
            $update['transport_title'] = $transport_title;
            $update['goods_freight'] = $freight;
            $update['goods_rules'] = $_POST['g_body'];
            $update['goods_mobile_rules'] = $_POST['m_body'];
        ...
        if($model_goods->editSampleGoods($update,$common_id)){
            showDialog('修改样品信息成功', $_POST['ref_url'], 'succ');
        }else{
            showDialog('修改样品信息失败');
        }
    }
    ...
    //独家经销权
    public function  onlybuy_powerOp(){
    	$commonid=$_GET["commonid"];
    	$data_list=Model("goods")->getGoodsOnlyBuyMember(array("goods_commonid"=>$commonid));
    	Tpl::output("data_list",$data_list);
    	$this->profile_menu('onlybuy_power', 'onlybuy_power');
    	Tpl::showpage("store_goods_onlybuy_power");
    }
    //获取独家经销有权限的会员组织机构
    public function getAllOnlyBuyPowerOrgOp(){
    	$commonid=$_GET["commonid"];
    	//获取该商品的 经销区域码
    	$area_data=Model("goods")->getGoodsOnlyBuyArea(array("goods_commonid"=>$commonid,"area_code"=>array("gt",0)),"area_code");
    	if(!empty($area_data)){
    		$conditon=array();
    		if(!empty($_GET["org_name"])){
    			$conditon["org_name"]=array("like","%".$_GET["org_name"]."%");
    		}
    		$condition_array=array();
    		$keys=array();
    		foreach ($area_data as $val){
	    		$area_code=$val["area_code"];
	    		if(substr($area_code,-4)=='0000'){//省
	    			$condition_array[]=array("like",substr($area_code,0,2)."____");
	    		}else if(substr($area_code,-2)=='00'){//市
	    			$condition_array[]=array("like",substr($area_code,0,4)."__");
	    		}else{//区、县
	    			$condition_array[]=$area_code;
	    		}
	    		$keys[]="area_code";
    		}
    		...
    		if(count($keys)>1){
    			$condition_array["_multi"]=true;
    			$conditon[implode("|",$keys)]=$condition_array;
    			$data_list=$model_member->getMemberOrgList($conditon,"org_name,org_id,conneter,conneter_tel,area_name",15);
    		}else{
    			$conditon["area_code"]=$condition_array[0];
    			$data_list=$model_member->getMemberOrgList($conditon,"org_name,org_id,conneter,conneter_tel,area_name",15);
    		}
    	...
    	Tpl::output("data_list",$data_list);   
    	Tpl::showpage("store_goods_onlybuy_power_add");
    }
    //添加商品独家经销会员
    public function addOnlyBuyPowerOrgOp(){
    	$org_id=$_GET["org_id"];
    	$commonid=$_GET["commonid"];
        if(empty($org_id)||empty($commonid)){
        	showDialog("参数错误", '', 'error');
        }
        $model_member=Model("member");
        $member_org=$model_member->getMemberOrg(array("org_id"=>$org_id));
        if(empty($member_org)){
        	showDialog("参数错误", '', 'error');
        }
        $data=array();
        $data["goods_commonid"]=$commonid;
        $data["member_orgid"]=$org_id;
        $data["member_orgname"]=$member_org["org_name"];
        $data["conneter"]=$member_org["conneter"];
        $data["conneter_tel"]=$member_org["conneter_tel"];
        $data["area_code"]=$member_org["area_code"];
        $data["area_name"]=$member_org["area_name"];
        $data["create_time"]=TIMESTAMP;
        $result=Model("goods")->addGoodsOnlyBuyMember($data);
        if($result){
        	redirect("index.php?act=store_goods_online&op=onlybuy_power&commonid=".$commonid);
        }else{
        	showDialog("设置失败", '', 'error');
        }
    }
    public function delOnlyBuyPowerOrgOp(){
    	$id=$_GET["id"];
        $commonid=$_GET["commonid"];
        $url="index.php?act=store_goods_online&op=onlybuy_power&commonid=".$commonid;
        $result=Model("goods")->delGoodsOnlyBuyMember(array("id"=>$id));
        if($result){
        	redirect($url);
        }else{
        	showDialog("删除失败",$url, 'error');
        }
    }
}
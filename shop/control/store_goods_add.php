...
class store_goods_addControl extends BaseSellerControl {
    ...
    public function save_goodsOp() {
        if (chksubmit()) {
            ...
            $common_array['goods_tag']       = $_POST['g_tag'];
            ...
            $common_array['goods_vip_price']        = floatval($_POST['g_vip_price']);
            ...
            $common_array['goods_common_moq']   = intval($_POST['g_moq']);
            ...
            $common_array['pindan_common_rebates']         = floatval($_POST['p_common_rebates']);
            $common_array['pindan_success_num']         =intval($_POST['p_success_num']);
            $common_array['pindan_cycle']         = intval($_POST['p_cycle']);
            // 保存数据
 		...
 		require_once(BASE_RESOURCE_PATH.DS.'phpqrcode'.DS.'index.php');
                ...
                if (is_array($_POST['spec'])) {
                    foreach ($_POST['spec'] as $value) {
                        ...
                        $goods['goods_tag']         = $common_array['goods_tag'];
                        ...
                        $goods['goods_vip_price']       = $value['vipprice'];
                        $goods['pindan_goods_rebates']       = floatval($value['rebates']);
                        ...
                        $goods['goods_moq']         = intval($value['moq']);  
			 ...
			 // 生成商品二维码
                    ...
                    $goods['goods_vip_price']       = $common_array['goods_vip_price'];
                    $goods['pindan_goods_rebates']       = floatval($common_array['pindan_common_rebates']);
                    ...
                    $goods['goods_moq']         = $common_array['goods_common_moq'];   
                ...
                // 生成拼单活动
                $pindan=array();
                $pindan["goods_commonid"]=$common_id;
                $pindan["start_time"]=TIMESTAMP;
                $pindan["end_time"]=TIMESTAMP+$common_array['pindan_cycle']*60*60;
                $pindan["success_num"]=$common_array['pindan_success_num'];
                ...
                if (isset($_POST['starttime'])) {
                    ...
                    if ($selltime > TIMESTAMP) {
                        ...
                        $pindan["start_time"]=$selltime;
                        $pindan["end_time"]=$selltime+$common_array['pindan_cycle']*60*60;
                    }
                }
                if($common_array['pindan_success_num']>0){//大于0创建拼单 0代表不拼单
                   Model("pindan")->createPindan($pindan);
                }
                // 记录日志

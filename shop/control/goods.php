...
class goodsControl extends BaseGoodsControl {
    ...
    public function indexOp() {
        ...
        $goods_detail = $model_goods->getGoodsDetail($goods_id,!empty($_SESSION["member_level"])&&$_SESSION["member_level"]==2);
        $goods_info = $goods_detail['goods_info'];
        if (empty($goods_info)) {
            showMessage(L('goods_index_no_goods'), '', 'html', 'error');
        }
        if($goods_info['goods_state']=='0'){
            showMessage('该商品已下架', '', 'html', 'error');
        }
        //检查是否独家权限
        if($_SESSION["is_login"]){
            if(!$model_goods->checkOnlyBuyPower($goods_info,$_SESSION["org_id"],$_SESSION["area_code"])){
            	showMessage("您没有权限查看该商品", '', 'html', 'error');
            }
        }
	// by abc.com
	...
	//  添加 end
        $sample_goods_info = $model_goods->getSampleGoodsInfo(array('goods_commonid'=>$goods_detail['goods_info']['goods_commonid']));
        Tpl::output('sample_goods_info',$sample_goods_info);
        $this->getStoreInfo($goods_info['store_id']);
        Tpl::output('spec_goods_moq',json_encode($goods_detail['spec_goods_moq']));   
        ...
        Tpl::output('pindan_info',$goods_detail['pindan_info']);
        //热销排行
        $goods_ranking=$model_goods->getGoodsRanking();
        Tpl::output('hot_sales', $goods_ranking["hot_sales"]);
        
        //收藏排行
        Tpl::output('hot_collect', $goods_ranking["hot_collect"]);
        ...
        $store_join = Model('store')->table('store_joinin')->where(array('member_id'=>$_SESSION['member_id']))->field('company_name,company_address')->find();
        Tpl::output('store_join', $store_join);
        Tpl::showpage('goods');
    }
    /**
     * 记录浏览历史
    ...
    public function salelogOp() {
        ...
        if ($_GET['vr']) {
            ...
            $sales = $model_order->getOrderAndOrderGoodsSalesRecordList(array('order_goods.goods_id'=>$goods_id), 'order_goods.*, order.buyer_name, order.add_time,order.member_org_name', 10);
        }
        Tpl::output('show_page',$model_order->showpage());
        Tpl::output('sales',$sales);
        Tpl::output('show_price',$this->show_price());
    ...
    public function ajax_goods_spec_listOp(){
    	$commonid=$_GET["commonid"];
    	$goods_model=Model("goods");
    	$commoninfo=$goods_model->getGoodeCommonInfo(array("goods_commonid"=>$commonid),"goods_name,goods_common_moq");
    	$spec_list=Model("goods")->getGoodsSpecListByCommonId($commonid);
    	$goods_model->setGoodsPrice($spec_list);
    	foreach ($spec_list as $i=>$row){
    		$s_array=unserialize($row['goods_spec']);
    		unset($spec_list[$i]['goods_spec']);
    		...
    		$spec_list[$i]["goods_spec_name"]=implode(' ',$tmp_spec_val_array);
    	}
        exit(json_encode(array("spec_list"=>$spec_list,"common_info"=>$commoninfo)));
    }

...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        if(empty($_SESSION["org_id"])){
        	showMessage("抱歉，您还未申请店铺未认证！</br>请先店铺认证，才能采购。", 'index.php?act=member_org', 'html', 'error');
        }else{
            $member_org_info=Model("member")->getMemberOrg(array("org_id"=>$_SESSION["org_id"]),"auth_statu");
            if($member_org_info["auth_statu"]==0){
            	showMessage("抱歉，您申请店铺认证正在审核中！", 'index.php?act=member_org', 'html', 'error');
            }
            if($member_org_info["auth_statu"]==2){
            	showMessage("抱歉，您申请店铺认证审核未通过！", 'index.php?act=member_org', 'html', 'error');
            }
        }
        //虚拟商品购买分流
        ...
        //得到购买数据
        $logic_buy = Logic('buy');
        $result = $logic_buy->buyStep1($_POST['cart_id'], $_POST['ifcart'], $_SESSION['member_id'], $_SESSION['store_id'],$_POST['sample_goods']);
        if(!$result['state']) {
        ...
        //样品采购
        Tpl::output('is_sample',$result['is_sample']);
//        foreach($result['store_cart_list'] as $key =>$storev){
//            foreach ($storev as $v ) {
//                if($v['goods_tag']=='保税仓商品'){
//                    $result['store_cart_list'][$key]['is_bondedwarehouse'] =1;
//                    break;
//                }
//            }
//        }
//        log::record(var_dump($result['store_cart_list']));

        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        Tpl::output('store_cart_list', $result['store_cart_list']);
        log::record(json_encode($result['store_cart_list']));



        //Tpl::output('is_bondedwarehouse', $is_bondedwarehouse);
    ...
    public function buy_step2Op() {
        ...
        $result = $logic_buy->buyStep2($_POST, $_SESSION['member_id'], $_SESSION['member_name'], $_SESSION['member_email'], $_SESSION['org_id'], $_SESSION['org_name']);
        if(!$result['state']) {
    ...
    public function payOp() {
        ...
        if ($pay_amount_online > 0) {
            ...
            //获取用户余额
            $pd_amount=$this->member_info["available_predeposit"];
            if($pd_amount>=$pay_amount_online){
	...
	public function pay_okOp() {
	    ...
	    //查询支付单信息
	    $model_order= Model('order');
	    if($_GET['pay_amount']==0.00){
	    	$data['pay_sn']=$pay_sn;
	    	$paydata=$model_order->getOrderInfo($data);
	    	
	    	$_GET['pay_amount']=$paydata['order_amount'];
	    }

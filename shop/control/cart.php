...
class cartControl extends BaseBuyControl {
	...
	public function addOp() {
        ...
        if (isset($_GET['goods_id'])) {
            $goods_ids_array=explode(",",$_GET['goods_id']);
            if($_GET['commonid']){
            	if(!$model_cart->common_checkcart($_GET['commonid'],$goods_ids_array,$_SESSION['member_id'])){
            		$remark_text="该商品您曾加入了其他规格";
            	}
            }
            $quantity_array=explode(",",$_GET['quantity']);
            $index=0;
            foreach($goods_ids_array as $goods_id){
                 //商品加入购物车(默认)
                 $quantity = intval($quantity_array[$index++]);
                 $goods_info=$model_goods->getGoodsOnlineInfoAndPromotionById($goods_id); //读取商品所有信息
                ...
                //VIP会员
                 ...
                 $goods_info_array[]=array('g'=>$goods_info,'q'=>$quantity);
            }
			...
			$goods_info['goods_tag'] = $bl_info['bl_tag'];
			...
			//log::record(json_encode($goods_info));
            if($_SESSION['member_id']){
             $goods_info['member_id']=$_SESSION['member_id'];
            }
            $quantity = 1;
            $goods_info_array[]=array('g'=>goods_info,'q'=>$quantity);
        }
        ...
        foreach($goods_info_array as $item){
          $model_cart->addCart($item['g'],$save_type,$item['q']);
        }
        //if ($insert) {
            //购物车商品种数记入cookie
            setNcCookie('cart_goods_num',$model_cart->cart_goods_num,2*3600);
            $data = array('state'=>'true', 'num' => $model_cart->cart_goods_num, 'amount' => ncPriceFormat($model_cart->cart_all_price),'text'=>$remark_text);
        //} else {
		...
		if ($cart_info['bl_id'] == '0') {
		    ...
		    if(empty($goods_info)) {
		        ...
		        exit(json_encode($return));
                    ...
                    if(intval($goods_info['goods_moq'])>$goods_info['goods_num']){
		        $return['state'] = 'min_invalid';
                        $return['msg']='最小起订量为'.$goods_info['goods_moq'];
		        $return['goods_num'] = $cart_info['goods_num'];
		        exit(json_encode($return));
                    }
		    //抢购
		    $logic_buy_1->getGroupbuyInfo($goods_info);
	...
	public function delOp() {
        ...
        if ($_SESSION['member_id']) {
            //登录状态下删除数据库内容
            $delete	= $model_cart->delCart('db',array('cart_id'=>array('in',$cart_id),'buyer_id'=>$_SESSION['member_id']));
            if($delete) {
                $data['state'] = 'true';

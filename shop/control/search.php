...
class searchControl extends BaseHomeControl {
    ...
    public function indexOp() {
        ...
        if (in_array($getkey,array('1','2','3'))) {
            $sequence = $getorder == '1' ? 'asc' : 'desc';
            $order = str_replace(array('1','2','3'), array('goods_salenum','goods_click','goods_promotion_price'), $getkey);
            $order .= ' '.$sequence;
        }
        ...
        if (is_array($indexer_ids)) {
            ...
            if(empty($_SESSION["area_code"])){
                $condition["is_onlybuy"]=0;
            }else{
            	$onlybuy_power_ids=$model_goods->getOnlyBuyPowerGoodsCommonIds($_SESSION["org_id"],$_SESSION["area_code"]);
            	if(!empty($onlybuy_power_ids)){
            	  $condition["is_onlybuy|is_onlybuy"]=array(0,array("exp","is_onlybuy=1 and goods_commonid in (".implode(',',$onlybuy_power_ids).")"),"_multi"=>true);
            	}else{
            	  $condition["is_onlybuy"]=0;
            	}
            }

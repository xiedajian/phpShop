...
class paymentControl extends BaseHomeControl{
	...
	public function real_orderOp(){
        ...
        if($payment_code=="otherpay"){//找人代付
        	$buyControl=new BaseBuyControl();
        	Tpl::output('buy_step','step3');
        	Language::read('home_cart_index');
            Tpl::output("order_info",$result['data']);
            ...
            $otherpay_sn=encrypt((empty($_SESSION["member_truename"])?$_SESSION["member_name"]:$_SESSION["member_truename"]).",".$order_ids_str);
            Tpl::output("otherpay_sn",$otherpay_sn);
        	Tpl::showpage("otherpay");
            //redirect("otherpay");
        }else if($payment_code=="transfer"){//线下转账
        	$buyControl=new BaseBuyControl();
        	...
        	Language::read('home_cart_index');
        	Tpl::output('hidden_rtoolbar_cart', 1);
        	Tpl::showpage("transfer_pay");
        }else if($payment_code=="predeposit"){
        	$member_id=$_SESSION["member_id"];
        	$memberinfo=Model("member")->getMemberInfoByID($member_id,"member_paypwd,available_predeposit");
        	if(md5($_POST["paypwd"])!=$memberinfo['member_paypwd']){
        		showMessage('支付密码错误', "index.php?act=buy&op=pay&pay_sn=".$pay_sn, 'html', 'error');
        	}
        	if($memberinfo["available_predeposit"]<$result['data']["api_pay_amount"]){
        		showMessage('余额不足', "index.php?act=buy&op=pay&pay_sn=".$pay_sn, 'html', 'error');
        	}
        	$order_sns=array();
        	foreach ($result['data']["order_list"] as $order){
        	...
        	//减少余额
        	$pddata=array();
        	$pddata["member_id"]=$member_id;
        	$pddata["member_name"]=$_SESSION["member_name"];
			...
			$order_list =$model_order->getNormalOrderList(array("order_sn"=>$pddata['order_sn']));
			if($order_list[0][order_type]==2){
				$model_licences=Model("licences");
				$result=$model_licences->update_licences_order($payment_info['payment_code'], $order_list);
			}else{
				$logic_payment->updateRealOrder($pay_sn,$payment_code, $result['data']["order_list"],$pay_sn);
			}

        	//跳转到成功页
        	redirect("index.php?act=buy&op=pay_ok&pay_sn=".$pay_sn."&pay_amount=".$result['data']["api_pay_amount"]);
        }else{
           //转到第三方API支付
	...
	private function _api_pay($order_info, $payment_info) {
    	...
    	if($payment_info['payment_code'] == 'chinabank'||$payment_info['payment_code'] == 'fuiou') {
    		$payment_api->submit();
    	} else {
	...
	public function notifyOp(){
		...
		if ($order_type == 'real_order') {
			if($order_list[0]["order_type"]==2){
				$model_licences=Model("licences");
				$result=$model_licences->update_licences_order($payment_info['payment_code'], $order_list, $trade_no);
			}else{
	...
	public function returnOp(){
		...
		if ($payment_state != 'success') {
            ...
            //更改订单支付状态
			if($order_type == 'real_order') {
				if($order_list[0]["order_type"]==2){  //付费创建的订单
					$model_licences=Model("licences");
					$result=$model_licences->update_licences_order($payment_info['payment_code'], $order_list, $trade_no);
				}else{
					$result = $logic_payment->updateRealOrder($out_trade_no, $payment_info['payment_code'], $order_list, $trade_no);

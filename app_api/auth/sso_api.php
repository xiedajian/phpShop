...
include_once BASE_PATH.'/auth/aes.php';
...
function sync_login($callback = ''){
    ...
    $url = SSO_AUTH_API.'?m=sso&c=sso&a=sync_login&callback='.$callback.'&app_id=3&data='.$data;
    return $url;
    //header('location:'.$url);
}
function getUserInfo($ticket){
    ...
    $data = _encode($data,SSO_APP_KEY);
    $data = base64_encode($data);
    $url = SSO_AUTH_API."?m=sso&c=sso&a=get_user_info&app_id=3&data=".$data;

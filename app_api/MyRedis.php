...
class MyRedis{
     ...
     * @param int $post
     */
    public function __construct($host = '192.168.1.252', $port = 6379) {
    ...
    public function set_expire($key, $value, $timeOut=0) {
        $this->redis->set($key, $value);
        if ($timeOut > 0)
     ...
     * 生成token,7天失效
     */
    public function  access_token($member_id,$user_info){
        $secretKey = '32213c157b6491e6c7b7ccea09e8eeae';
        $createTime = time();
        $token = sha1($member_id . $secretKey . $createTime);
        $old_token = $this->get($member_id);
        //删除
        if(!empty($old_token)){
            $this->del($old_token);
        }
        $this->set_expire($member_id,$token,24*60*60*7);  //设置一天过期
        $this->set_expire($token,$user_info,24*60*60*7);
        return $token;
    }
}
?>
...
function drop_cart_items(){
    var i=0;
    var id_arr=[];
    var ids_str ='';
    $("input[id^='cart_id']:checked").each(function(){
        id_arr[i++] =$(this).attr('id').substr(7);
        ids_str +=$(this).attr('id').substr(7)+',';
    });
    if(id_arr.length==0){
        showDialog('请选择商品', 'alert', '');return;
    }
    showDialog('确认删除'+i+'个商品吗?', 'confirm', '', function(){
            $.getJSON('index.php?act=cart&op=del&cart_id=' + ids_str, function(result){
                if(result.state){
                    //删除成功
                    for(var x in id_arr){
                        var  cart_id = id_arr[x];
                        var parent_tr = $('#cart_item_' + cart_id).parent();
                        if(result.quantity == 0){//判断购物车是否为空
                            window.location.reload();    //刷新
                        } else {
...
$(function(){
    ...
    $('input[nc_type="eachGoodsCheckBox"]').on('click',function(){
        if (!$(this).attr('checked')) {
            ...
            $(this).closest('tbody').children('tr:eq(0)').find(':checkbox').prop('checked',false);
        }
        calc_cart_price();
    });
    ...
    //店铺全选
    $('.store_select_all').click(function(){
        if($(this).prop('checked')){
            $(this).closest('tbody').find(':checkbox').prop('checked',true);
        } else{
            $(this).closest('tbody').find(':checkbox').prop('checked',false);
        }
        calc_cart_price();
    });

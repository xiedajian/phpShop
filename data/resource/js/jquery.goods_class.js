			...
			$.ajax({
				...
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].gc_states==1){
						src += " | <a href=\"javascript:if(confirm('确定禁用此商品分类？'))window.location = 'index.php?act=goods_class&op=goods_class_states&gc_id="+data[i].gc_id+"&gc_states="+data[i].gc_states+"';\">禁用</a>";
						}else{
							src += " | <a href=\"javascript:if(confirm('确定启用此商品分类？'))window.location = 'index.php?act=goods_class&op=goods_class_states&gc_id="+data[i].gc_id+"&gc_states="+data[i].gc_states+"';\">启用</a>";
						}
						src += "</td>";
						src += "</tr>";
					}

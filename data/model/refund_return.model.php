...
class refund_returnModel extends Model{
	...
	public function addRefundReturn($refund_array, $order = array(), $goods = array()) {
		...
		$refund_array['update_time'] = TIMESTAMP;
	    ...
	    if (!empty($order_list) && is_array($order_list)) {
    	    foreach ($order_list as $key => $value) {
        	    ...
        	    if ($order_state == $order_paid && $payment_code != 'offline') {//已付款未发货的非货到付款订单可以申请取消
                    if($order_refund){
                        if(empty($order_list[$order_id]['refund_list'])){
    ...
    //拆单商品退款
    public function addRefundGoodsList($goods_list,$order_info){
        $ret = false;
        $refund_money = 0;
        $all_goods = array();
        $all_goods_num = array();
        foreach($order_info['extend_order_goods'] as $value){
            $all_goods[$value['goods_id']] =$value;
            $all_goods_num[$value['goods_id']] =$value['goods_num'];
        }
        foreach($goods_list as $goods_id=>$good_num){
            $data = array();
            $data['order_id'] = $order_info['order_id'];
            $data['order_sn'] = $order_info['order_sn'];
            $data['refund_sn'] = $this->getRefundsn($order_info['store_id']);
            $data['store_id'] = $order_info['store_id'];
            $data['store_name'] = $order_info['store_name'];
            $data['buyer_id'] = $order_info['buyer_id'];
            $data['buyer_name'] = $order_info['buyer_name'];

            $data['goods_id'] = $all_goods[$goods_id]['goods_id'];
            $data['order_goods_id'] = $all_goods[$goods_id]['rec_id'];
            $data['goods_name'] = $all_goods[$goods_id]['goods_name'];
            $data['goods_num'] = $good_num;
            $data['refund_amount'] = $good_num* $all_goods[$goods_id]['goods_price'];
            $data['goods_image'] = $all_goods[$goods_id]['goods_image'];
            $data['order_goods_type'] = $all_goods[$goods_id]['goods_type'];

            $data['refund_type'] = 1;//退款
            $data['seller_state'] = 2;//同意
            $data['refund_state'] = 2;//待管理员处理
            $data['return_type'] = 1;
            $data['order_lock'] = 1;
            $data['goods_state'] = 1;
            $data['add_time'] = TIMESTAMP;
            $data['seller_time'] = TIMESTAMP;

            $data['reason_id'] = 0;
            $data['reason_info'] = '商品缺货做退款处理';
            $data['buyer_message'] = '';
            $data['seller_message'] = '商品缺货做退款处理';
            $ret =$this->table('refund_return')->insert($data);
            $refund_money += $data['refund_amount'];
        }
        $refund_list = $this->getRefundList(array('order_id'=>$order_info['order_id']));
        $refund_goods = array();
        foreach($refund_list as $val){
            if($refund_goods[$val['goods_id']]){
                $refund_goods[$val['goods_id']]+=$val['goods_num'];
            }else{
                $refund_goods[$val['goods_id']] = $val['goods_num'];
            }
        }
        $all_refund = true;
        foreach($all_goods_num as $k=>$v){
            if(!isset($refund_goods[$k])){
                $all_refund = false;break;
            }
            if($refund_goods[$k]!=$v){
                $all_refund = false;break;
            }
        ...
        if($all_refund){
            $data['refund_state']  =2;
            $data['order_state'] = 40;//交易完成
        }else{
            $data['refund_state']  =1;
        }
        $this->table('order')->where(array('order_id'=>$order_info['order_id']))->update($data);
        return $ret;
    }

...
class areaModel extends Model {
    ...
    /*更新area_array.js文件*/
    public function updateAreaJs(){
        $js = 'd:/wamp/www/area_array_new.js';
        $file = fopen($js,'w+');
        if(!$file){
            echo 'file not found:'.$js;exit;
        }
        fwrite($file,'nc_a = new Array();');
        $data = $this->getCache();
        $arr = array();
        foreach ($data['children'] as $k => $v) {
            $childs = array();
            foreach ($v as $vv) {
                $childs[] = '[\''.$vv.'\',\''. $data['name'][$vv].'\']';
                //$arr[$k][] = array($vv, $data['name'][$vv]);
            }
            fwrite($file,"nc_a[$k]=[".implode($childs,',')."];\r\n");
        }
        fclose($file);
    }

...
class goodsModel extends Model{
     ...
     * @return array
     */
    public function getGoodsCommonList($condition, $field = '*', $page = 10, $order = 'sort desc,goods_commonid desc') {
        $condition = $this->_getRecursiveClass($condition);
    ...
    public function getGoodsSpecListByCommonId($common_id) {
        ...
        if (empty($spec_list)) {
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_id,store_id,goods_image,color_id,goods_moq,goods_vip_price,goods_price,goods_vip_price,goods_storage');
     ...
     * @return array
     */
    public function getGoodsDetail($goods_id,$is_vip=false) {
        if($goods_id <= 0) {
            return null;
        }
        ...
        $result1["goods_image"]=cthumb($result1['goods_image'],60, $result1['store_id']);
        $result2 = $this->getGoodeCommonInfoByID($result1['goods_commonid']);
        $result2["common_goods_name"]=$result2["goods_name"];
        $result2["common_goods_image"]=cthumb($result2['goods_image'],60, $result2['store_id']);
        ...
        foreach ($spec_array as $key => $value) {
            ...
            $tpl_spec['name']=empty($s_array)?$goods_info["goods_name"]:implode(' ',$tmp_spec_val_array);
            $tpl_spec['url'] = urlShop('goods', 'index', array('goods_id' => $value['goods_id']));
            $tpl_spec['image']=cthumb($value['goods_image'],60, $goods_info['store_id']);
            $tpl_spec['moq']=$value["goods_moq"];
            $tpl_spec['price']=$value["goods_price"];
            $tpl_spec['vip_price']=$value["goods_vip_price"];
            $tpl_spec['id']=$value["goods_id"];
            $tpl_spec['storage']=$value["goods_storage"];
            
            $spec_list[] = $tpl_spec;
            $spec_list_mobile[$spec_sign] = $value['goods_id'];
            $spec_image[$value['color_id']] = thumb($value, 60);
            //$spec_goods_moq[$value['goods_id']]=$value['goods_moq'];
        }
        ...
        //显示vip价格
        $goods_info['show_vip_price'] = false;
        $price = isset($goods_info['promotion_price'])&&$goods_info['promotion_price']>0 ? $goods_info['promotion_price']:$goods_info['goods_price'];
        if($goods_info['goods_vip_price']>0&&$goods_info['goods_vip_price']<$price&&$_SESSION['member_level']==2){
            $goods_info['show_vip_price'] = true;
        }
        //显示价格
        $goods_info['show_price'] = true;
        if($_SESSION['is_login']!=1||empty($_SESSION['member_id'])){
            $goods_info['show_price'] = false;
            $goods_info['show_price_tip'] = '请先登录后查看价格&nbsp;<a href="index.php?act=login" style="color: #E1017E;">马上登录</a>';
        }
        else if($_SESSION['auth_statu']!=1){
            $goods_info['show_price'] = false;
            $goods_info['show_price_tip'] = '请先认证后查看价格&nbsp;<a href="index.php?act=member_org&op=index" style="color: #E1017E;">马上认证</a>';
        }
        if($_SESSION['store_id'] == $goods_info['store_id']){
            $goods_info['show_price'] = true;
        }
        ...
        //拼单信息
        if($goods_info["pindan_id"]){
        	$pindan_info=Model('pindan')->getPindanInfo(array("pindan_id"=>$goods_info["pindan_id"]));
        	if(!empty($pindan_info)){
        		$time_down=$pindan_info["end_time"]-TIMESTAMP;
        		$pindan_info["time_down"]=$time_down>0?$time_down:600;
        		$result['pindan_info']=$pindan_info;
        	}
        }
        //$result['spec_goods_moq']=$spec_goods_moq;
        return $result;
    }
	...
	public function getMobileBodyByCommonID($goods_commonid, $fields = 'mobile_body') {
     ...
     //商品排行(销售、排行)
     public function goodsRanking($type,$limit=10){
     	//删除原有数据
     	$this->table("goods_ranking")->where(array("type"=>$type))->delete();
     	//获取数据
     	$stat_common_list=DB::getAll("select sum(".($type==1?"goods_salenum":"goods_collect").") as num,count(*) as spec_count,goods_id,goods_name,goods_image,goods_spec,goods_price,goods_commonid from (SELECT * from 33hao_goods ORDER BY goods_price asc) as A group by goods_commonid ORDER BY num DESC LIMIT ".$limit);
     	$data_list=array();
     	foreach ($stat_common_list as $value){
            $s_array = unserialize($value['goods_spec']);
            unset($value['goods_spec']);
            $tmp_spec_val_array=array();
            if (!empty($s_array) && is_array($s_array)) {
            	foreach ($s_array as $k => $v) {
            		$tmp_spec_val_array[]=$v;
            	}
            	$value["goods_name"]=str_replace(" ".implode(' ',$tmp_spec_val_array),"",$value["goods_name"]) ;
            }
            $value["type"]=$type;
            $data_list[]=$value;
     	}
     	//添加数据
     	$this->table("goods_ranking")->insertAll($data_list);
     }
     //获取商品排行
     public function getGoodsRanking(){
     	$data_list=$this->table("goods_ranking")->field("*")->order("num desc")->select();
     	$hot_sales=array();
     	$hot_collect=array();
     	...
     	return array("hot_sales"=>$hot_sales,"hot_collect"=>$hot_collect);
     }
     //获取  商品活动价格
     public function setGoodsPrice(& $goods_array,$is_vip=false){
     	//限时折扣价格
     	$goods_ids=array();
     	foreach ($goods_array as $val){
     	...
     	$xianshi_goods_price=Model('p_xianshi_goods')->getXianshiGoodsList($condition, $page=null, $order='', $field='goods_id,xianshi_price');
        if(!empty($xianshi_goods_price)){
        	$xianshi_goods_price_array=array();
        	foreach ($xianshi_goods_price as $v){
        		$xianshi_goods_price_array[$v["goods_id"]]=$v["xianshi_price"];
        	}
        	foreach ($goods_array as $i=>$goods){
        		if(isset($xianshi_goods_price_array[$goods["goods_id"]])&&$xianshi_goods_price_array[$goods["goods_id"]]>0){
        			$goods_array[$i]["goods_price"]=$xianshi_goods_price_array[$goods["goods_id"]];
        		}
        	}
        }
     	//VIP会员价格
     	if($is_vip){
     		foreach ($goods_array as $i=>$goods){
     		   if($goods["goods_vip_price"]>0&&$goods["goods_vip_price"]<$goods["goods_price"]){
     		   	  $goods_array[$i]["goods_price"]=$goods["goods_vip_price"];
     		   }
     		}
     	}
     }
    ...
    //获取样品
    public function getSampleGoodsInfo($condition = array(),$field = '*'){
        return $this->table('sample_goods')->field($field)->where($condition)->find();
    }
    //修改样品信息
    public function editSampleGoods($update = array(),$goods_commonid){
        $condition = array('goods_commonid'=>$goods_commonid);
        if($this->table('sample_goods')->where($condition)->count()>0){
            $result = $this->table('sample_goods')->where($condition)->update($update);
        }else{
            $result = $this->table('sample_goods')->where($condition)->insert($update);
        }
        return $result;
    }
    ...
    //独家经销 权限的商品ID
    public function getOnlyBuyPowerGoodsCommonIds($org_id,$member_code){
    	if(strlen($member_code)!=6) return null;
    	//获取所有权限的
    	$area_condition=array();
    	if(substr($member_code,-4)=='0000'){//省
    		$area_condition["area_code"]=array("like",substr($member_code,0,2)."____");
    	}else if(substr($member_code,-2)=='00'){//市
    		$area_condition["area_code|area_code"]=array(substr($member_code,0,2)."0000",array("like",substr($member_code,0,4)."__"),"_multi"=>true);
    	}else{//区、县
    		$area_condition["area_code"]=array("in",array(substr($member_code,0,2)."0000",substr($member_code,0,4)."00",$member_code));
    	}
       	$area_power=$this->getGoodsOnlyBuyArea($area_condition,"goods_commonid");
       	if(empty($area_power)){
       		return null;
       	}
       	$goods_commonids=array();
       	foreach ($area_power as $val){
       		$goods_commonids[]=$val["goods_commonid"];
       	}
    	
    	//获取无权限的 商品ID
    	$member_condition=array();
    	$member_condition["member_orgid"]=array("neq",$org_id);
    	$member_condition["area_code"]=$member_code;
    	$member_power=$this->getGoodsOnlyBuyMember($member_condition,"goods_commonid");
    	if(empty($member_power)){
    		return $goods_commonids;
    	}
    	$nopower_commonids=array();
    	foreach ($member_power as $val){
    		$nopower_commonids[]=$val["goods_commonid"];
    	}
    	$result_commonids=array();
    	foreach ($goods_commonids as $id){
    		if(!in_array($id,$nopower_commonids)){
    			$result_commonids[]=$id;
    		}
    	}
    	return $result_commonids;
    }
    //是否有 独家经销权限
    public function checkOnlyBuyPower($goods,$org_id,$member_code){
    	if($goods["is_onlybuy"]==0){//不是独家经销商品
    		return true;
    	}
    	if(empty($org_id)||empty($member_code)){
    		return false;
    	}
    	$area_condition=array();
    	$area_condition["goods_commonid"]=$goods["goods_commonid"];
    	if(substr($member_code,-4)=='0000'){//省
    		$area_condition["area_code"]=array("like",substr($member_code,0,2)."____");
    	}else if(substr($member_code,-2)=='00'){//市
    		$area_condition["area_code|area_code"]=array(substr($member_code,0,2)."0000",array("like",substr($member_code,0,4)."__"),"_multi"=>true);
    	...
    	$onlybuy_area=$this->getGoodsOnlyBuyArea($area_condition,"id");
    	if(empty($onlybuy_area)){
    		return false;
    	}
    	//获取无权限的 商品ID
    	$member_condition=array();
    	$member_condition["member_orgid"]=array("neq",$org_id);
    	$member_condition["area_code"]=$member_code;
    	$member_condition["goods_commonid"]=$goods["goods_commonid"];
    	$member_power=$this->getGoodsOnlyBuyMember($member_condition,"id");
    	if(!empty($member_power)){
    		return false;
    	}
    	return true;
    ...
    //获取独家经销区域
    public  function  getGoodsOnlyBuyArea($condition,$field="*"){
        return $this->table("goods_onlybuy_area")->field($field)->where($condition)->select();
    }  
    public function  addGoodsOnlyBuyArea($data){
       $result=$this->table("goods_onlybuy_area")->insert($data);
       if($result){
       	  $this->editGoods(array("is_onlybuy"=>1),array("goods_commonid"=>$data["goods_commonid"]));
       	  $this->editGoodsCommon(array("is_onlybuy"=>1),array("goods_commonid"=>$data["goods_commonid"]));
       }
       return $result;
    }
    public function delGoodsOnlyBuyAreaById($id,$commonid){
    	$result=$this->table("goods_onlybuy_area")->where(array("id"=>$id,"goods_commonid"=>$commonid))->delete();
    	if($result){
    		$checkIds=$this->getGoodsOnlyBuyArea(array("goods_commonid"=>$commonid),"id");
    		if(empty($checkIds)){
    			$this->editGoods(array("is_onlybuy"=>0),array("goods_commonid"=>$commonid));
    			$this->editGoodsCommon(array("is_onlybuy"=>0),array("goods_commonid"=>$commonid));
    			$this->delGoodsOnlyBuyMember(array("goods_commonid"=>$commonid));
    		}
    	}
    	return $result;
    }
    public function  getGoodsOnlyBuyMember($condition,$field="*"){
    	return $this->table("goods_onlybuy_member")->field($field)->where($condition)->select();
    }
    public function  addGoodsOnlyBuyMember($data){
    	$result=$this->getGoodsOnlyBuyMember(array("goods_commonid"=>$data["goods_commonid"],"member_orgid"=>$data["member_orgid"]),"id");
    	if(empty($result)){
        	return $this->table("goods_onlybuy_member")->insert($data);
    	}else{
    		return true;
    	}
    }
    public function delGoodsOnlyBuyMember($condition){
    	return $this->table("goods_onlybuy_member")->where($condition)->delete();
    }

    /**
     * 查询商品的主商品名称
     */
    public function getGoodsCommonName($goods_ids = array()) {
        $condition = array();
        $condition['goods_id'] = array('in',$goods_ids);
        $list = $this->table('goods,goods_common')->field('goods.goods_id,goods_common.goods_name')->join('inner')->on('goods.goods_commonid = goods_common.goods_commonid')->where($condition)->select();
        foreach($list as $val){
            $ret[$val['goods_id']] = $val['goods_name'];
        }
        return $ret;
    }

    public function updateGoodsSort($goods_commonid,$sort){
        if($goods_commonid<=0){
            return false;
        }
        try{
            ...
            $data = array();
            $data['sort'] = $sort;
            $result = $this->table('goods_common')->where($condition)->update($data);
            if($result===false){
                throw new Exception();
            }
            $result = $this->table('goods')->where($condition)->update($data);
            if($result===false){
                throw new Exception();
            }
            $this->commit();

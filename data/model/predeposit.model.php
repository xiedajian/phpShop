...
class predepositModel extends Model {
    ...
    public function addRechargeCard($sn, array $session)
    {
        ...
        if (empty($card)) {
            throw new Exception("抱歉！不存在此卡号");
        }
        if($card['state'] != 0){
            throw new Exception("抱歉！卡号已过期");
        }
        if ($card['member_id'] != 0) {
            throw new Exception("啊！慢了一步！卡号被人充走啦");
        }
    ...
    public function changePd($change_type,$data = array()) {
        ...
        switch ($change_type){
            ...
            case 'cash_del':
				...
				case 'sys_add_money':
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【增加】，充值单号: '.$data['pdr_sn']."。".$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
				...
				case 'sys_del_money':
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【减少】，充值单号: '.$data['pdr_sn']."。".$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = -$data['amount'];
                $data_log['lg_freeze_amount'] = $data['amount'];
				$data_log['lg_desc'] = '管理员调节预存款【冻结】，充值单号: '.$data['pdr_sn']."。".$data['lg_desc'];
				$data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit-'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit+'.$data['amount']);
                ...
                $data_log['lg_av_amount'] = $data['amount'];
                $data_log['lg_freeze_amount'] = -$data['amount'];
                $data_log['lg_desc'] = '管理员调节预存款【解冻】，充值单号: '.$data['pdr_sn']."。".$data['lg_desc'];
                $data_log['lg_admin_name'] = $data['admin_name'];
                $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                $data_pd['freeze_predeposit'] = array('exp','freeze_predeposit-'.$data['amount']);
                ...
                case 'pindan_rebates'://拼单返现
                    $data_log['lg_av_amount'] = $data['amount'];
                    $data_log['lg_desc'] = '订单:'.$data['order_sn'].',交易完成.商品【'.$data['goods_name'].'】返现'.number_format($data['amount'],2).'元.';
                    $data_pd['available_predeposit'] = array('exp','available_predeposit+'.$data['amount']);
                    $data_pd['pindan_rebates_total']=array('exp','pindan_rebates_total+'.$data['amount']);
                    $data_msg['av_amount'] = $data['amount'];
                    $data_msg['freeze_amount'] = 0;
                    $data_msg['desc'] = $data_log['lg_desc'];
                    break;
    ...
    public function addWithdrawAccount($data){
        return $this->table('withdraw_account')->insert($data);
    }
     ...
     * @return mixed
     */
    public function delWithdrawAccount($condition = array()){
        return $this->table('withdraw_account')->where($condition)->delete();
    }
     ...
     * @return array
     */
    public function getWithdrawAccountList($member_id){
        ...
        return $this->table('withdraw_account')->where($condition)->select();
    }
    public function getWithdrawAccount($condition = array(),$fields = '*'){
        return $this->table('withdraw_account')->field($fields)->where($condition)->find();
    }

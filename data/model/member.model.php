...
class memberModel extends Model {
    ...
    //获取会员列表  包含组织架构
    public function getMemberJoinOrgList($condition = array(), $field = '*', $page = 0, $order = 'member_id desc', $limit = ''){
    	return $this->table('member,member_org')->join('left')->on('member.org_id=member_org.org_id')->field($field)->where($condition)->page($page)->order($order)->limit($limit)->select();
    }
    /**
     * 会员数量
     * @param array $condition
	...
	public function  getMemerCountfilter($condition){
		return $this->table('member,member_org')->join('left')->on('member.org_id=member_org.org_id')->where($condition)->count();
	}
    ...
    public function createSession($member_info = array(),$reg = false) {
        ...
        $_SESSION['org_id']     = $member_info['org_id'];
        $_SESSION['member_level']     = $member_info['member_level'];
        $_SESSION['member_group']     = $member_info['member_group'];
        if($member_info['org_id']){
        	$org_info=$this->getMemberOrg(array('org_id'=>$member_info['org_id']),"org_name,auth_statu,area_code");
        	$_SESSION['org_name']=$org_info['org_name'];
        	$_SESSION['auth_statu']=$org_info['auth_statu'];
        	$_SESSION['area_code']=$org_info['area_code'];
        }
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function addMember($param,$add_to_umall = true) {
		if(empty($param)) {
			return false;
		}
		try {
		    ...
		    $member_info['register_client']		= $param['register_client'];
		    $member_info['mobile_address']		= $param['mobile_address'];
		    $member_info['member_group']		= $param['member_group']?$param['member_group']:1;
            ...
            $member_info["member_id"]=$insert_id;
			if($add_to_umall){
				$ret = add_umall_user($member_info);
				if($ret['status']!==0){
					Log::record('umall同步注册失败：'.$ret['msg']);
					throw new Exception();
				}
			}
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function addMemberf($param) {
		if(empty($param)) {
			return false;
		}
		try {
			...
			$member_info['register_client']		= $param['register_client'];
			$member_info['mobile_address']		= $param['mobile_address'];
			$member_info['member_group']		= $param['member_group'];
			//$member_info['member_qqinfo']		= $param['member_qqinfo'];
			//$member_info['member_sinaopenid']	= $param['member_sinaopenid'];
			//$member_info['member_sinainfo']	= $param['member_sinainfo'];
			//添加邀请人(推荐人)会员积分 by abc.com
	...
	//添加 会员组织机构
	public function addMemberOrg($data){
        return $this->table('member_org')->insert($data);
	}
	public function getMemberOrg($condition, $field = '*') {
		return $this->table('member_org')->field($field)->where($condition)->find();
	}
	public function editMemberOrg($condition,$data){
		return $this->table('member_org')->where($condition)->update($data);
	}
	public function getMemberOrgList($condition, $field = '*',$page = 0){
		return $this->table('member_org')->where($condition)->page($page)->select();
	}
	//按审核时间排序
	public function getMemberOrgListsh($condition, $field = '*',$page = 0){
		return $this->table('member_org')->where($condition)->order('auth_time desc')->page($page)->select();
	}


    /**修改密码**/
    public function editPassword($member_id='',$mobile,$password){
        $condition = array();
        if($member_id){
            $condition['member_id'] = $member_id;
        }else if($mobile){
        ...
        if(count($condition)>0){
            $data = array('member_passwd'=>md5($password));
            $update = $this->table('member')->where($condition)->update($data);
        }
        return $update;
    }
    
    public function addMemberAction($data){
    	$this->table("member_action")->insert($data);
    }

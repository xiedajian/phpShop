...
class cartModel extends Model {
	...
	private function _addCartDb($goods_info = array(),$quantity) {
		...
		$array['goods_tag'] = $goods_info['goods_tag'];
    ...
    public function common_checkcart($commonid,$goods_ids,$buyer_id){
    	$model_goods=Model("goods");
    	$goods_list=$model_goods->getGoodsList(array("goods_commonid"=>$commonid),"goods_id");
    	$goods_id_array=array();
    	foreach ($goods_list as $value){
    		$goods_id_array[]=$value["goods_id"];
    	}
    	$cart_goods_list=$this->table("cart")->field("cart_id,goods_id")->where(array("buyer_id"=>$buyer_id,"goods_id"=>array("in",$goods_id_array)))->select();
    	if(empty($cart_goods_list)) return true;
    	$at_cart_ids=array();
    	$del_cart_ids=array();
        foreach ($cart_goods_list as $val){
        	if(in_array($val["goods_id"],$goods_ids)){
        		$del_cart_ids[]=$val["cart_id"];
        	}else{
        		$at_cart_ids[]=$val["cart_id"];
        	}
        }
        $this->table("cart")->where(array("cart_id"=>array("in",$del_cart_ids)))->delete();
        if(count($at_cart_ids)>0){
        	return false;
        }
        return true;
    } 
    //检查  新手商品
//     public function checkNewerGoods($member_id,$cart_list){
//     	$newergoods_count=0;
//         $newer_goods="";
//     	foreach ($cart_list as $cart){
//     		if(strpos($cart["goods_name"],"新手")>0){
//     			$newergoods_count++;
//     			$newer_goods=$cart;
//     		}
//     	}
//     	if($newergoods_count==0) return true;
//     	$buyer_power=$this->table("member_action")->field("action_id")->where(array("member_id"=>$member_id,"type"=>1))->select();
//     	if(!empty($buyer_power)){
//     		return "抱歉,您已购买过新手商品,不能再次购买了！";
//     	}
//     	if($newergoods_count>1){
//     		return "抱歉，只能购买一个新手商品";
//     	}
//     	$matches=array();
//     	if(preg_match("/限(\d+)/", $newer_goods["goods_name"], $matches)){
//     	    if(intval($matches[1])<$newer_goods["goods_num"]){
//     	    	return "抱歉,".$newer_goods["goods_name"]."该新手商品限购".$matches[1];
//     	    }
//     	}
//     	return true;
//     }
    ...
    public function checkNewerGoods($member_id,$cart_list){
    	...
    	$member_action_list=$this->table("member_action")->field("content")->where(array("member_id"=>$member_id,"type"=>1))->select();
    	if(!empty($member_action_list)){
    		$order_ids=array();
    		foreach ($member_action_list as $val){
    			$content=unserialize($val["content"]);
    			$order_ids[]=$content["order_id"];
    		}
    		$order_list=$this->table("order")->field("order_state")->where(array("order_id"=>array("in",$order_ids)))->select();
    		foreach($order_list as $v){
    			if($v["order_state"]!=0){//已取消的订单  可以通过
    				return "抱歉,您已购买过新手商品,不能再次购买了！";
    			}
    		}
    	}

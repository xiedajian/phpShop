        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                if($store_info['settlement']==1){
                    ...
                    $commis_amount_sum=floatval($order_goods_info['commis_amount']);
                    ...
                    if($seller_money>0){
                        ...
                        $condition['seller_money'] = array('elt',0);
                        $update_result = $model_order->table('order')->where($condition)->update(array('seller_money'=>$seller_money));
                ...
                //支付了  拼单返利
                Model("pindan")->pindan_rebates(array($order_info));
                if (!$result) {
...
//         if (empty($pd_payment_info)) return;
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['dlyp_id'])) {
            ...
            $reciver_info['postcode'] = $address_info['postcode'];
            $reciver_info = serialize($reciver_info);
    ...
    private function _getGoodsOnlineInfo($goods_id,$quantity) {
        ...
        $new_array['goods_vip_price'] = $goods_info['goods_vip_price'];
        ...
        $new_array['goods_moq']=$goods_info['goods_moq'];
    ...
    public function getGroupbuyInfo(& $goods_info = array()) {	
    	...
    	$model_order = Model('order');
    		...
    		//判断用户是否已经购买过本商品，如果购买过则不享受抢购优惠
    		if(sizeof($_SESSION)>0){
    			$data['order.buyer_id']=$_SESSION['member_id'];
    		}else{
    			...
    			$data['order.buyer_id']=$mb_user_token_info['member_id'];
    		}
    		$data['goods_id']=$goods_info['goods_id'];
    		$data['add_time']=array('time',array($goods_info['groupbuy_info']['start_time'],$goods_info['groupbuy_info']['end_time']));
    		 
    		$getlist=$model_order->getOrderAndOrderGoodsSalesRecordListX($data);
    ...
    /**VIP会员修改价格,vip价格比最终价格低时使用vip价格**/
    public function getVipInfo( & $goods_info){
        global $wap_member_info;
        if($_SESSION['member_level']==2||$wap_member_info['member_level']==2){
            if($goods_info['goods_vip_price']>0&&$goods_info['goods_vip_price']<$goods_info['goods_price']){
                $goods_info['goods_price'] = $goods_info['goods_vip_price'];
                $goods_info['goods_pay_type'] = 2;
            }
        }
    }

    /**VIP会员修改价格,vip价格比最终价格低时使用vip价格**/
    public function getVipList( & $goods_list){
    ...
    private function _getOnlineCartList($cart_list) {
        ...
        foreach ((array)$cart_list as $key => $cart_info) {
            ...
            if (in_array($cart_info['goods_id'],array_keys($goods_online_array))) {
                ...
                $cart_list[$key]['goods_vip_price'] = $goods_online_info['goods_vip_price'];
                ...
                $cart_list[$key]['goods_tag'] = $goods_online_info['goods_tag'];
    ...
    public function getGroupbuyCartList(& $cart_list) {
        ...
        foreach ($cart_list as $key => $cart_info) {
            ...
            //判断用户是否已经购买过本商品，如果购买过则不享受抢购优惠
          //  $data['order.buyer_id']=$_SESSION['member_id'];
           // $data['goods_id']=$cart_info['goods_id'];
            //$data['add_time']=array('time',array($cart_info['groupbuy_info']['start_time'],$cart_info['groupbuy_info']['end_time']));

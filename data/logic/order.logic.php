...
class orderLogic {
    ...
    public function changeOrderStateReceive($order_info, $role, $user = '', $msg = '') {
        try {
            ...
            //收货 拼单返现
            Model("pindan")->pindan_rebates_do_by_orderid($order_id);
            //添加订单日志
            $data = array();
            $data['order_id'] = $order_id;
            $data['log_role'] = 'buyer';
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array()) {
        ...
        $param['param'] = array(
            'shipping_code' =>  $post['shipping_code'],//快递单号
            'express_name' => $express_info['e_name'],//快递名称
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        try {
            ...
            foreach($order_list as $order_info) {
                ...
                //--供应商结算方式--一付款即可提现
                $store_ids[] = $order_info['store_id'];
                $store_order[$order_info['store_id']] = $order_info;
                //--供应商结算方式--一付款即可提现
            }
            //--供应商结算方式--一付款即可提现
            if(count($store_ids)>0){
                //付款即可提现-获取店铺
                $store_list = Model('store')->table('store')->field('store_id,member_id,member_name')->where(array('store_id'=>array('in',$store_ids),'settlement'=>1))->select();
                if($store_list){
                    foreach($store_list as $store_info){
                        if(isset($store_order[$store_info['store_id']])){
                            $temp_order_info = $store_order[$store_info['store_id']];//可提现订单
                            //计算订单金额
                            $refund = Model('refund_return')->table('refund_return')->where(array('order_id'=>$temp_order_info['order_id'],'refund_state'=>3))->find();
                            $seller_money = 0;//订单收入金额
                            if($refund){
                                $seller_money=$temp_order_info['order_amount']-$refund['refund_amount'];
                            }else{
                                $seller_money=$temp_order_info['order_amount'];
                            }
                            //取得拥金金额
                            $field = 'SUM(ROUND(goods_pay_price*commis_rate/100,2)) as commis_amount';
                            $order_goods_condition = array();
                            $order_goods_condition['order_id'] = $temp_order_info['order_id'];
                            ...
                            if($seller_money>0){
                                $condition = array();
                                $condition['order_id'] = $temp_order_info['order_id'];
                                ...
                                if($update_result>0){
                                    $is_offline=($temp_order_info['payment_code']=="offline");
                                    if ($seller_money > 0 && $is_offline == false) {
                                        ...
                                        $data['pdr_sn'] = $temp_order_info['order_sn'];
            ...
            //支付 订单后  拼单返利
            Model("pindan")->pindan_rebates($order_list);
            $model_order->commit();
        } catch (Exception $e) {
            $model_order->rollback();
            return callback(false,$e->getMessage());

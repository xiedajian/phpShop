...
class buyLogic {
     ...
     * @param unknown $sample_goods  是否拿样
     * @return Ambigous <multitype:unknown, multitype:unknown >
     */
    public function buyStep1($cart_id, $ifcart, $member_id, $store_id,$sample_goods = '') {
        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id);
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $store_id,$sample_goods);
            if($sample_goods == 1){//样品购买
                $result['data']['is_sample'] = true;
            }
        }
        if(!$result['state']) {
            return $result;
    ...
    public function getCartList($cart_id, $member_id) {
        ...
        $newer_goods_check=$model_cart->checkNewerGoods($member_id,$cart_list);
        if($newer_goods_check!==true){
        	return callback(false,$newer_goods_check);
        }
        //购物车列表 [得到最新商品属性及促销信息]
    ...
    public function getGoodsList($cart_id, $member_id, $store_id,$sample_goods='') {
        ...
        if($sample_goods == 1){//样品购买
            $goods_info = $this->_sampleGoodsInfo($goods_info);
            if($goods_info['state']===false){
                return callback(false, $goods_info['msg']);
            }
        }
        if(empty($goods_info)) {
    ...
    public function getBuyStep1Data($member_id, $data) {
        ...
        $goods_common_moq_check=array();
        foreach($goods_list as $g){
          if($g['goods_commonid']){
             $goods_common_ids[]=$g['goods_commonid']; 
             $goods_common_moq_check[$g['goods_commonid']]+=$g['goods_num'];
          }
        }
        //购买数量限制
        if($data['is_sample']!=1){
            $goods_common_ids=array_unique($goods_common_ids);
            $goods_common_moqs=Model('goods')->getGoodsCommonList(array('goods_commonid'=>array('in',$goods_common_ids)),'goods_commonid,goods_name,goods_common_moq',0,'');
            foreach($goods_common_moqs as $m){
                if(intval($m['goods_common_moq'])>$goods_common_moq_check[$m['goods_commonid']]){
                    return callback(false,'【'.$m['goods_name'].'】</br>未达到'.$m['goods_common_moq'].'的混批量');
                }
            }
        }else{//样品
            if(count($goods_common_ids)==1){
                $sample_goods_info = Model('goods')->getSampleGoodsInfo(array('goods_commonid'=>$goods_common_ids[0]));
                if($sample_goods_info['num_limit']<$goods_common_moq_check[$sample_goods_info['goods_commonid']]){
                    return callback(false,'【'.$sample_goods_info['goods_name'].'】</br>购买样品数量不能超过'.$sample_goods_info['num_limit'].'件');
                }
            }else{
                return callback(false,'请求参数错误');
            }
        }
        //定义返回数组
        $result = array();
        $result['is_sample'] = $data['is_sample'];
     ...
     * @return array
     */
    public function buyStep2($post, $member_id, $member_name, $member_email,$member_org_id,$member_org_name) {
        ...
        $this->_member_info['org_id']=$member_org_id;
        $this->_member_info['org_name']=$member_org_name;
    ...
    private function _createOrderStep2() {
        ...
        if ($post['ifcart']) {
            ...
            if($this->_post_data['is_sample']==1){
                $goods_info = $this->_sampleGoodsInfo($goods_info);
            }
            if($goods_info['state']===false) {
                throw new Exception($goods_info['msg']);
            }
    ...
    private function _createOrderStep4() {
        ...
        $member_org_id= $this->_member_info['org_id'];
        $member_org_name= $this->_member_info['org_name'];
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            $order['member_org_id']=$member_org_id;
            $order['member_org_name']=$member_org_name;
            ...
            foreach ($goods_list as $goods_info) {
                ...
                if (!intval($goods_info['bl_id'])) {
                    ...
                    $order_goods[$i]['goods_pay_type'] = $goods_info['goods_pay_type']?$goods_info['goods_pay_type']:0;
                    ...
                    //如果包含新手商品 加入成员动作表
                    if(strpos($goods_info['goods_name'],'新手')>0){
                    	$order_goods[$i]['goods_pay_type']=4;
                    	$member_action=array();
                    	$member_action["type"]=1;
                    	$member_action["create_time"]=TIMESTAMP;
                    	$member_action["member_id"]=$member_id;
                    	$member_action["content"]=serialize(array("goods_id"=>$goods_info['goods_id'],"goods_name"=>$goods_info['goods_name'],"goods_price"=>$goods_info['goods_price'],"goods_num"=>$goods_info['goods_num'],"order_id"=>$order_id));
                    	Model("member")->addMemberAction($member_action);
                    }
                    $i++;
                    //存储库存报警数据
    ...
    private function _getGoodsList($cart_list) {
        ...
        foreach ($cart_list as $key => $cart) {
            ...
            if (!intval($cart['bl_id'])) {
                ...
                $goods_list[$i]['goods_commonid']=$cart['goods_commonid'];
    ...
    /*判断样品信息*/
    private function _sampleGoodsInfo($goods_info){
        $model_goods = Model('goods');
        $model_order = Model('order');
        $sample_goods_info = $model_goods->getSampleGoodsInfo(array('goods_commonid'=>$goods_info['goods_commonid'],'enable'=>1));
        if(empty($sample_goods_info)){
            return callback(false, '样品已下架或不存在');
        }
        //判断购买样品数量限制
        $num = $model_order->getBuySampleGoodsNum($goods_info['goods_commonid'],$_SESSION['member_id']);
        if($num+$goods_info['goods_num']>$sample_goods_info['num_limit']){
            $can_buy = $sample_goods_info['num_limit'] - $num;
            if($can_buy<0){
                $can_buy = 0;
            }
            return callback(false, '样品限制购买'.$sample_goods_info['num_limit'].'件，您本次还能购买'.$can_buy.'件');
        }
        $goods_info['goods_price'] = $sample_goods_info['goods_price'];
        $goods_info['goods_name'] = $sample_goods_info['goods_name']?$sample_goods_info['goods_name']:$goods_info['goods_name'];
        $goods_info['transport_id'] = $sample_goods_info['transport_id'];
        $goods_info['goods_freight'] = $sample_goods_info['goods_freight'];
        $goods_info['goods_num'] = $sample_goods_info['num_limit']< $goods_info['goods_num'] ? $sample_goods_info['num_limit']:$goods_info['goods_num'];
        $goods_info['goods_pay_type'] = 3;//样品价购买
        return $goods_info;
    }
}

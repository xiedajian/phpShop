$(function() {
	...
	if(ifcart==1){
		...
		var sample_goods = GetQueryString("sample_goods");
        if(sample_goods!=1){
            sample_goods = 0;
        }
		var data = {key:key,cart_id:cart_id,ifcart:0,sample_goods:sample_goods};
	}
	...
	$.ajax({//提交订单信息
		...
		success:function(result){
			...
			$.each(data.store_cart_list,function(k,v){//循环店铺
				...
				htmldata+='<p class="buys-yt-tlt">店铺名称：'+v.store_name+'<span data-store_id="'+k+'" class="store-cod-supported" style="display:none;">（该店铺不支持选定收货地址的货到付款）</span></p>';
						$.each(v.goods_list,function(k1,v1){//循环商品列表
							...
							str='';
							if(v1.goods_tag=='保税仓商品'){
								str ='<span class="bondedwarehouse">保税仓发货</span>';
							}
                        ...
                        htmldata+='<div class="msg_client"><span style="vertical-align: top">买家留言：</span><textarea name="pay_message" class="msg-content" width="200px" maxlength="150" store_id="'+k+'" ></textarea></div>';
				if(v.is_bondedwarehouse){
					htmldata+= '<div class="bondedware">尊敬的拼单网客户，您好。根据海关相关政策规定，每2件保税仓商品对应一套身份信息，请您在留言中正确备注身份信息，为避免耽误您接收商品，身份证号码请填写正确，证件号码必须与证件姓名完全对应。如有疑问请联系在线客服或者拨打我们的客服热线4000908982。</div>';
				}
                         htmldata+= '<p>运费：￥<span id="store'+k+'"></span></p>';
			...
			if(data.address_info == ''){//收获地址是否存在
                ...
                $('#step1_postcode').html(data.address_info.postcode);
			}
...
*/
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		for(var i=0;i<data.address_list.length;i++){
                    var post_code_string ='';
                    var  post_code_temp = data.address_list[i].postcode =='undefined'?'':data.address_list[i].postcode;
                    if(post_code_temp){
                        post_code_string = ' <span class="step1_postcode_'+data.address_list[i].address_id+'">['+post_code_temp+']</span>';
                    }
        ...
        if(address_id>0){//变更地址
            ...
            $.ajax({
        		...
        		success:function(result){
                    ...
                    $('#step1_postcode').html($('.step1_postcode_'+address_id).html());
			...
			if($.sValid()){
				...
				$.ajax({
					...
					data:{key:key,true_name:true_name,mob_phone:mob_phone,tel_phone:tel_phone,city_id:city_id,area_id:area_id,address:address,area_info:area_info,postcode:postcode},
					dataType:'json',
					success:function(result){
						if(result){
							$.ajax({//获取收货地址信息
								...
								success:function(result1){
                                    ...
                                    $('#step1_postcode').html(data1.address_info.postcode);
    ...
    $('#buy_step2').click(function(){//提交订单step2
        ...
        if(sample_goods == 1){//样品订单
            data.sample_goods = sample_goods;
        }else{
            data.sample_goods = 0;
        }
...
*/
        //买家留言
        var message_store_id = [];
        var message = [];
        $("textarea[name=pay_message]").each(function(){
            var store_id = $(this).attr('store_id');
            if(store_id){
                message_store_id.push(store_id);
                message.push($(this).val());
            }
        });
        data.message_store_id = message_store_id.join(',');
        ...
        $.ajax({
        	...
        	success:function(result){
        		...
        		if(result.datas.order_ids != ''){
        			location.href = WapSiteUrl+'/tmpl/member/order_pay.html?order_id='+result.datas.order_ids;
        		}
        		return false;
        	}
        });

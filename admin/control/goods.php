...
class goodsControl extends SystemControl{
    ...
    //独家经销
    public function onlybuy_areaOp(){
    	$commonid=$_GET["goods_commonid"];
    	$data_list=Model("goods")->getGoodsOnlyBuyArea(array("goods_commonid"=>$commonid));
    	Tpl::output("data_list",$data_list);
        Tpl::showpage('goods.onlybuy_area', 'null_layout');
    }
    
    public function onlybuy_area_addOp(){
    	$goods_commonid=$_GET["commonid"];
    	$area_id=$_GET["area_id"];
    	$area_info=Model("area")->getAreaInfo(array("area_id"=>$area_id));
    	if(!empty($area_info)){
    		$insertid=Model("goods")->addGoodsOnlyBuyArea(array("goods_commonid"=>$goods_commonid,"area_code"=>$area_info["area_code"],"area_name"=>$area_info["mergername"],"create_time"=>TIMESTAMP));
    		if($insertid){
    		    exit(json_encode(array("success"=>true,"area_name"=>$area_info["mergername"],"id"=>$insertid,"goods_commonid"=>$goods_commonid)));
    	    }else{
    ...
    public function onlybuy_area_delOp(){
      exit(json_encode(array("success"=>Model("goods")->delGoodsOnlyBuyAreaById($_GET["id"],$_GET["commonid"]))));
    }

    //商品排序
    public function sortOp(){
        $commonid = intval($_POST['commonid']);
        $sort = intval($_POST['val']);
        $result = false;
        if($commonid>0){
            $model_goods = Model('goods');
            $result = $model_goods->updateGoodsSort($commonid,$sort);
        }
        echo $result?1:0;
    }
}

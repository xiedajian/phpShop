...
class stat_storeControl extends SystemControl{
    ...
    public function newstoreOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
		    ...
		    $excel_obj->addWorksheet($excel_obj->charset('新增供应商统计',CHARSET));
		    $excel_obj->generateXML($excel_obj->charset('新增供应商统计',CHARSET).date('Y-m-d-H',time()));
			exit();
		} else {
			//得到统计图数据
    		$stat_arr['title'] = '新增供应商统计';
            $stat_arr['yAxis'] = '新增供应商数';
    ...
    public function degreeOp(){
    	...
    	if(!empty($storelist)){
    		foreach ($storelist as $k=>$v){
    			$storelist[$k]['p_name'] = $v['grade_id'] > 0?$sd_list[$v['grade_id']]:'平台供应商';
    			$storelist[$k]['allnum'] = intval($v['allnum']);
    			$statlist['headertitle'][] = $v['grade_id'] > 0?$sd_list[$v['grade_id']]:'平台供应商';
	...
	public function showstoreOp(){
        ...
        if ($_GET['exporttype'] == 'excel'){
		    ...
		    $excel_data[0][] = array('styleid'=>'s_title','data'=>'供应商账号');
		    $excel_data[0][] = array('styleid'=>'s_title','data'=>'供应商卖家账号');
			...
			$excel_data = $excel_obj->charset($excel_data,CHARSET);
			$excel_obj->addArray($excel_data);
		    $excel_obj->addWorksheet($excel_obj->charset('新增供应商',CHARSET));
		    $excel_obj->generateXML($excel_obj->charset('新增供应商',CHARSET).date('Y-m-d-H',time()));
			exit();
        }
	...
	public function storesales_listOp(){
		...
		//列表header
		$statheader = array();
        $statheader[] = array('text'=>'供应商名称','key'=>'store_name');
	...
	public function storeareaOp(){
		...
		foreach ((array)$province_array as $k=>$v){
		    if ($statlist[$k]){
		        $stat_arr[] = array('cha'=>$k,'name'=>$v,'des'=>"，供应商量：{$statlist[$k]['storenum']}",'level'=>$statlist[$k]['level']);
		    } else {

...
class storeControl extends SystemControl{
     ...
     * 结算方式修改
     */
    public function edit_settlementOp(){
        $store_id = intval($_POST['id']);
        $val = intval($_POST['val']);
        $value_array = array(0,1,2,3);
        if($store_id>0&&in_array($val,$value_array)){
            $condition = array('store_id'=>$store_id);
            $update = array();
            $update['settlement'] = $val;
            $update['update_time'] = time();
            echo Model('store')->editStore($update,$condition);
        }
    }
	/**
	 * 店铺
	...
	public function storeOp(){
        ...
        if(intval($_GET['settlement']) > 0){
            $condition['settlement']		= intval($_GET['settlement']);
            Tpl::output('settlement',intval($_GET['settlement']));
        }
    ...
    private function store_joinin_verify_open($joinin_detail) {
        ...
        if($_POST['verify_type'] === 'pass') {
           ...
           //同步到umall
            ...
            if($store_id) {
//                 $store['ipvp_member_id']=$_POST['member_id'];
//                 $store['company']=$shop_array['store_company_name'];
//                 $store['name']=$shop_array['store_name'];
//                 $ret = add_umall_store($store);
                //写入卖家账号
                $seller_array = array();
			...
			if($state) {
                ...
                $model_member->editMember(array('member_id'=>$joinin_detail['member_id']),array('member_group'=>4));
				// 添加相册默认
    ...
    //同步店铺到umall上
    //数据格式转换
    private function _user_ipvp_to_umall($member){
    	if(empty($member['member_truename'])||empty($member['member_mobile'])||empty($member['member_passwd'])||empty($member['ipvp_member_id'])){
    		return null;
    	}
    	$user = array();
    	$user['nickname'] = $member['member_truename'];
    	$user['phone'] = $member['member_mobile'];
    	$user['password'] = $member['member_passwd'];
    	$user['ipvp_member_id'] = $member['ipvp_member_id'];
    	return $user;
    }
    
    private function _data_encode($data){
    	$data = json_encode($data);
    	$data = base64_encode($data);
    	return $data;
    }
    private function _data_decode($data){

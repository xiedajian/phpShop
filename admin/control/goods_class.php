...
class goods_classControl extends SystemControl{
	...
	//启用/禁用商品分类
	public function goods_class_statesOp(){
		$model_class = Model('goods_class');
		
		if(intval($_GET['gc_states'])==1){
			$model_class->editGoodsClass(array('gc_states'=>0),array('gc_id'=>intval($_GET['gc_id'])));
			showMessage('操作成功','index.php?act=goods_class&op=goods_class');
		}else{
			$model_class->editGoodsClass(array('gc_states'=>1),array('gc_id'=>intval($_GET['gc_id'])));
			showMessage('操作成功','index.php?act=goods_class&op=goods_class');
		}

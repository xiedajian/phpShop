    ...
    <table class="table tb-type2">
      <thead>
        <tr class="thead">
          <th class="w24"></th>
          <th class="w24"></th>
            <th class="w24 align-center">排序</th>
      ...
      <tbody>
        ...
        <tr class="hover edit"  data-commonid="<?php echo $v['goods_commonid'];?>">
          ...
          <td><i class="icon-plus-sign" style="cursor: pointer;" nctype="ajaxGoodsList" data-comminid="<?php echo $v['goods_commonid'];?>" title="点击展开查看此商品全部规格；规格值过多时请横向拖动区域内的滚动条进行浏览。"></i></td>
            <td class="align-center"><input class="sort-edit" data-old="<?php echo $v['sort'];?>" value="<?php echo $v['sort'];?>" style="width: 46px;border-radius: 5px;padding: 5px;border-style: solid;border-width: 1px;border-color: #ABADB3 #E2E3EA #E2E3EA #ABADB3;"></td>
...
$(function(){
    ...
    $('.sort-edit').change(function(){
        ...
        if(old!=now){
            var object = $(this);
            var commonid = $(this).closest('tr').data('commonid');
            $.post('index.php?act=goods&op=sort',{commonid:commonid,val:now},function(result){
                if(result<=0){
                    object.val(old);
                    alert('保存失败');
                }else{
...
function goods_onlybuy(goods_commonid){
    _uri = "<?php echo ADMIN_SITE_URL;?>/index.php?act=goods&op=onlybuy_area&goods_commonid=" + goods_commonid;
    CUR_DIALOG = ajax_form('goods_onlybuy', '经销区域设置', _uri, 500);
}

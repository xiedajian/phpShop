      ...
      <thead>
        <tr class="thead">
          ...
          <th><?php echo $lang['goods_class_add_commis_rate'];?></th>
    		<th>状态</th>
          <th></th>
          <th style='width:100px;'><?php echo $lang['nc_handle'];?></th>
        </tr>
      </thead>
      <tbody>
        <?php if(!empty($output['class_list']) && is_array($output['class_list'])){ ?>
            ...
            <tr class="hover edit">
             ...
             <?php if($v['gc_states']==1){ ?>
              ...
              <td class="w84"><a href="index.php?act=goods_class&op=goods_class_edit&gc_id=<?php echo $v['gc_id'];?>"><?php echo $lang['nc_edit'];?></a> | <a href="javascript:if(confirm('<?php echo $lang['goods_class_index_ensure_del'];?>'))window.location = 'index.php?act=goods_class&op=goods_class_del&gc_id=<?php echo $v['gc_id'];?>';"><?php echo $lang['nc_del'];?></a>
              | <?php if($v['gc_states']==1){  ?><a href="javascript:if(confirm('确定禁用此商品分类？'))window.location = 'index.php?act=goods_class&op=goods_class_states&gc_id=<?php echo $v['gc_id'];?>&gc_states=<?php echo $v['gc_states']; ?>';">禁用</td>
              <?php }else{ ?>
              <a href="javascript:if(confirm('确定启用此商品分类？'))window.location = 'index.php?act=goods_class&op=goods_class_states&gc_id=<?php echo $v['gc_id'];?>&gc_states=<?php echo $v['gc_states']; ?>';">启用</td>
              <?php } ?>
            </tr>
            <?php } ?>
        <?php }else { ?>
